...
static int audio_sipc_create(int target_id)
{
	...
	sp_asoc_pr_dbg("%s: ioremap txbuf: vbase=%#zx, pbase=0x%x, size=0x%x\n",
		 ...
		 int32_t timeout)
{
	...
	if (ret < 0) {
		if (-ENODATA == ret) {
			sp_asoc_pr_dbg("%s channel =%u cmd=%u ENODATA\n",
			 ...
			 int type, void *buf, size_t n, int32_t timeout)
{
	...
	/* send audio cmd */
	sp_asoc_pr_dbg("cmd =%d sharemem_info.id = %d, sharemem_info.type=%d,",
		cmd, sharemem_info.id, sharemem_info.type);
	sp_asoc_pr_dbg(" sharemem_info.phy_iram_addr=%#x, sharemem_info.size=%#x\n",
