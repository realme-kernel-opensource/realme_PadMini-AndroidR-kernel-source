...
extern int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info);
...
static void silfp_netlink_send(struct silfp_data *fp_dev, const int cmd)
{
    ...
    LOG_MSG_DEBUG(INFO_LOG, "[%s] kaoshan  send cmd %d\n", __func__, cmd);
...
static void silfp_get_screen_status(int status)
{
    ...
    if(fp_dev == NULL)
    {
        printk("kaoshan #### fp_dev is null return \n");
        return ;
    }
    ...
    if(status) {
        ...
        silfp_netlink_send(g_fp_dev, SIFP_NETLINK_SCR_OFF);
         ...
         LOG_MSG_DEBUG(INFO_LOG, "[%s] kaoshan  LCD ON\n", __func__);
        fp_dev->scr_off = 0;
...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
    case SIFP_IOC_INIT:
         printk("kaoshan %s %d start ####",__func__,__LINE__);
        ...
        printk("kaoshan %s %d end ####",__func__,__LINE__);
        g_fp_dev = fp_dev;
