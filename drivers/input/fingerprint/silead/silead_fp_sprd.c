...
static int silfp_parse_dts(struct silfp_data* fp_dev)
{
    ...
    printk("kaoshan %s %d ###",__func__,__LINE__);
...
int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
    ...
    fp_dev->vddio_port = of_get_named_gpio(fp_dev->spi->dev.of_node, "vddio-gpios", 0);
     printk("kaoshan %s %d fp_dev->avdd_port = %d ###",__func__,__LINE__,fp_dev->avdd_port);
    if (fp_dev->avdd_port >= 0 ) {
        gpio_free(fp_dev->avdd_port);
        ret = gpio_request(fp_dev->avdd_port, "SILFP_AVDD_PIN");
        if (ret < 0) {
            LOG_MSG_DEBUG(ERR_LOG, "[%s] fp_dev->avdd_port Failed to request GPIO=%d, ret=%d",__func__,(s32)fp_dev->avdd_port, ret);
            status = -ENODEV;
            goto err_avdd;
        }
    }
    ...
    LOG_MSG_DEBUG(INFO_LOG, "[%s] kaoshan int_port %d, rst_port %d.\n",__func__,fp_dev->int_port,fp_dev->rst_port);
    if (fp_dev->int_port >= 0 ) {
        gpio_free(fp_dev->int_port);
        printk("kaoshan %s %d gpio_free fp_dev->avdd_port = %d ###",__func__,__LINE__,fp_dev->int_port);
    }

    if (fp_dev->rst_port >= 0 ) {
        gpio_free(fp_dev->rst_port);
        printk("kaoshan %s %d gpio_free fp_dev->rst_port = %d ###",__func__,__LINE__,fp_dev->rst_port);
    }

    ret = gpio_request(fp_dev->int_port, "SILFP_INT_IRQ");
    if (ret < 0) {
        LOG_MSG_DEBUG(ERR_LOG, "[%s] SILFP_INT_IRQ  Failed to request GPIO=%d, ret=%d",__func__,(s32)fp_dev->int_port, ret);
        status = -ENODEV;
        goto err;
    } else {
    ...
    if (fp_dev->rst_port >= 0 ) {
        ...
        if (ret < 0) {
            LOG_MSG_DEBUG(ERR_LOG, "[%s] kaoshan  Failed to request GPIO=%d, ret=%d",__func__,(s32)fp_dev->rst_port, ret);
            status = -ENODEV;
            goto err_rst;
        } else {
            msleep(10);
