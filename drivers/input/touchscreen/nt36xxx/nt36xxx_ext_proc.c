...
#define BASELINE_TEST "baseline_test"
#define COORDINATE "coordinate"
#define DELTA "delta"
#define BASELINE "baseline"
#define MAIN_REGISTER "main_register"
#define DEBUG_LEVEL "debug_level"
#define GESTURE "double_tap_enable"
#define IRQ_DEPATH "irq_depth"
#define REGISTER_INFO "register_info"
#define FW_UPDATE "tp_fw_update"
#define GAME_SWITCH "game_switch_enable"
#define TP_LIMIT_ENABLE "tp_limit_enable"
#define DBG_MESSAGE_DIFF_ENABLE "dbg_message_diff_enable"
#define DBG_FINGER_DOWN_DIFF "finger_down_diff"
#define DBG_STATUS_CHANGE_DIFF "status_change_diff"
#define DBG_GESTURE_COORD_RECORD_ENABLE "dbg_gesture_coord_record_enable"
#define DBG_GESTURE_COORD_RECORD "dbg_gesture_coord_record"
#define DBG_GESTURE_COORD_ENABLE "dbg_gesture_coord_enable"

static struct proc_dir_entry *coordinate;
static struct proc_dir_entry *delta;
static struct proc_dir_entry *baseline;
static struct proc_dir_entry *main_register;
static struct proc_dir_entry *debug_level;
static struct proc_dir_entry *gesture;
static struct proc_dir_entry *irq_depath;
static struct proc_dir_entry *register_info;
static struct proc_dir_entry *fw_update;
static struct proc_dir_entry *game_switch;
static struct proc_dir_entry *tp_limit_enable;
static struct proc_dir_entry *dbg_message_diff_enable_entry;
static struct proc_dir_entry *dbg_finger_down_diff_entry;
static struct proc_dir_entry *dbg_status_change_diff_entry;
static struct proc_dir_entry *dbg_gesture_coord_enable_entry;
static struct proc_dir_entry *dbg_gesture_coord_record_enable_entry;
static struct proc_dir_entry *dbg_gesture_coord_record_entry;
...
/* dbg_message_diff_enable */
static ssize_t dbg_message_diff_enable_write(struct file *filp, const char __user *buf,
...
static const struct file_operations dbg_message_diff_enable_fops =
{
	.write = dbg_message_diff_enable_write,
	.owner = THIS_MODULE,
};

#define NVT_MMAP_DEBUG_FINGER_DOWN_DIFFDATA   0x263E4
#define NVT_MMAP_DEBUG_STATUS_CHANGE_DIFFDATA 0x2666C
static int32_t dbg_finger_down_diff_open(struct inode *inode, struct file *file)
{
...
static const struct file_operations dbg_finger_down_diff_fops = {
	.owner = THIS_MODULE,
	.open = dbg_finger_down_diff_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};

static int32_t dbg_status_change_diff_open(struct inode *inode, struct file *file)
{
	if (mutex_lock_interruptible(&ts->lock)) {
		return -ERESTARTSYS;
	}
...
static const struct file_operations dbg_status_change_diff_fops = {
	.owner = THIS_MODULE,
	.open = dbg_status_change_diff_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};

/* dbg_gesture_coord_record_enable */
static ssize_t dbg_gesture_coord_record_enable_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	unsigned int tmp = 0;
	int32_t ret = 0;
...
static const struct file_operations dbg_gesture_coord_record_enable_fops =
{
	.write = dbg_gesture_coord_record_enable_write,
	.owner = THIS_MODULE,
};

static int32_t gesture_buf[1024] = {0};
/* dbg_gesture_coord_record_open */
static int32_t c_dbg_gesture_coord_record_show(struct seq_file *m, void *v)
{
	struct debug_gesture_record_info *gesture_record = &ts->debug_gesture_record_info;
	uint32_t i = 0;
	char tmp[256] = {0};
...
const struct seq_operations dbg_gesture_coornidate_record_seq_ops = {
    .start  = c_start,
    .next   = c_next,
    .stop   = c_stop,
    .show   = c_dbg_gesture_coord_record_show
};
...
static int32_t dbg_gesture_coord_record_open(struct inode *inode, struct file *file)
{
	uint32_t input_x = 0;
	uint32_t input_y = 0;
	uint32_t i;
	...
	memset(&ts->debug_gesture_record_info, 0, sizeof(struct debug_gesture_record_info));
	...
	for (i = 0 ; i < gesture_buf[1] ; i++) {
		...
		ts->debug_gesture_record_info.coordinate[i].x = (uint16_t) input_x;
		ts->debug_gesture_record_info.coordinate[i].y = (uint16_t) input_y;
	}
	...
	return seq_open(file, &dbg_gesture_coornidate_record_seq_ops);
}

static const struct file_operations dbg_gesture_coord_record_fops = {
	.owner = THIS_MODULE,
	.open = dbg_gesture_coord_record_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};

/* dbg_gesture_coord_enable */
static ssize_t dbg_gesture_coord_enable_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	unsigned int tmp = 0;
	int32_t ret = 0;
...
static const struct file_operations dbg_gesture_coord_enable_fops =
{
	.write = dbg_gesture_coord_enable_write,
	.owner = THIS_MODULE,
};
/*******************************************************
Description:
...
*******************************************************/
/* coordinate */
static int32_t c_coordinate_show(struct seq_file *m, void *v)
{
...
const struct seq_operations coordinate_seq_ops = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
	.show   = c_coordinate_show
};

static int32_t coordinate_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &coordinate_seq_ops);
}

static const struct file_operations coordinate_fops = {
	.owner = THIS_MODULE,
	.open = coordinate_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = seq_release,
};
...
const struct seq_operations main_register_seq_ops = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
...
{
	return seq_open(file, &main_register_seq_ops);
}
 ...
 */
static ssize_t gesture_write(struct file *filp, const char __user *buf,size_t count, loff_t *ppos)
{
	int value = 0;
	char cmd[128] = {0};
...
static ssize_t gesture_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
	int ret = 0;
	int len;
	uint8_t *ptr = NULL;
...
/* tp_fw_update */
static ssize_t fw_update_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	unsigned int tmp = 0;
...
static const struct file_operations fw_update_fops =
{
	.write = fw_update_write,
	.owner = THIS_MODULE,
};
...
const struct seq_operations irq_depath_seq_ops = {
	.start  = c_start,
	.next   = c_next,
	.stop   = c_stop,
...
{
	return seq_open(file, &irq_depath_seq_ops);
}
 ...
 */
static ssize_t register_info_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	uint8_t tmp[10] = {0};
	char cmd[128] = {0};
...
static ssize_t register_info_read(struct file *file, char __user *buff,size_t count, loff_t *ppos)
{
	uint8_t *buf = NULL;
	uint8_t *ptr = NULL;
	uint8_t len = 0;
	...
	buf = (uint8_t *)kzalloc(sizeof(uint8_t)*(reg.len), GFP_KERNEL);
	if (buf == NULL) {
		NVT_ERR("failed to allocate memory for buf\n");
		goto fail;
	}

	ptr = (uint8_t *)kzalloc(sizeof(uint8_t)*(reg.len)*3+1, GFP_KERNEL);
	if (ptr == NULL) {
		NVT_ERR("failed to allocate memory for ptr\n");
		goto fail;
	}
	...
	/* read data */
	nvt_set_page(reg.addr);
	buf[0] = reg.addr & 0x7F;
	CTP_SPI_READ(ts->client, buf, reg.len + 1);
...
static const struct file_operations register_info_fops =
{
	.write = register_info_write,
	.read = register_info_read,
	.owner = THIS_MODULE,
};

/* game_switch_enable */
static ssize_t game_switch_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	unsigned int tmp = 0;
	char cmd[128] = {0};
...
static ssize_t game_switch_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t tmp[8] = {0};
...
static const struct file_operations game_switch_fops =
{
	.write = game_switch_write,
	.read = game_switch_read,
	.owner = THIS_MODULE,
};

/* tp_limit_enable */
static ssize_t tp_limit_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	uint8_t tmp[8] = {0};
...
static ssize_t tp_limit_write(struct file *filp, const char __user *buf,
		size_t count, loff_t *ppos)
{
	unsigned int tmp = 0;
	char cmd[128] = {0};
...
static const struct file_operations tp_limit_fops =
{
	.write = tp_limit_write,
	.read = tp_limit_read,
	.owner = THIS_MODULE,
};
...
int32_t nvt_extra_proc_init(void)
{
	...
	/*  customized proc node */
	if ((ts->touchpanel_proc == NULL) || (ts->debug_info == NULL)) {
		NVT_ERR("proc/touchpanel/ or proc/touchpanel/debug_info is NULL!\n");
		return -ENOMEM;
	}

	coordinate = proc_create(COORDINATE, 0664,
			ts->touchpanel_proc, &coordinate_fops);
	if (coordinate == NULL) {
		NVT_ERR("create proc/touchpanel/coordinate Failed!\n");
		return -ENOMEM;
	}

	delta = proc_create(DELTA, 0664,
	...
	baseline = proc_create(BASELINE, 0664,
	...
	main_register = proc_create(MAIN_REGISTER, 0664,
			ts->debug_info, &main_register_fops);
	if (main_register == NULL) {
		NVT_ERR("create proc/touchpanel/debug_info/main_register Failed!\n");
		return -ENOMEM;
	}

	debug_level = proc_create(DEBUG_LEVEL, 0664,
			ts->touchpanel_proc, &debug_level_fops);
	if (debug_level == NULL) {
		NVT_ERR("create proc/touchpanel/debug_level Failed!\n");
		return -ENOMEM;
	}

	gesture = proc_create(GESTURE, 0664,
			ts->touchpanel_proc, &gesture_fops);
	if (gesture == NULL) {
		NVT_ERR("create proc/touchpanel/double_tap_enable Failed!\n");
		return -ENOMEM;
	}

	irq_depath = proc_create(IRQ_DEPATH, 0444,
			ts->touchpanel_proc, &irq_depath_fops);
	if (irq_depath == NULL) {
		NVT_ERR("create proc/touchpanel/irq_depath Failed!\n");
		return -ENOMEM;
	}

	register_info = proc_create(REGISTER_INFO, 0664,
			ts->touchpanel_proc, &register_info_fops);
	if (register_info == NULL) {
		NVT_ERR("create proc/touchpanel/register_info Failed!\n");
		return -ENOMEM;
	}

	fw_update = proc_create(FW_UPDATE, 0664,
			ts->touchpanel_proc, &fw_update_fops);
	if (fw_update == NULL) {
		NVT_ERR("create proc/touchpanel/tp_fw_update Failed!\n");
		return -ENOMEM;
	}

	game_switch = proc_create(GAME_SWITCH, 0666,
			ts->touchpanel_proc, &game_switch_fops);
	if (game_switch == NULL) {
		NVT_ERR("create proc/touchpanel/game_switch_enable Failed!\n");
		return -ENOMEM;
	}

	tp_limit_enable = proc_create(TP_LIMIT_ENABLE, 0666,
			ts->touchpanel_proc, &tp_limit_fops);
	if (tp_limit_enable == NULL) {
		NVT_ERR("create proc/touchpanel/tp_limit_enable Failed!\n");
		return -ENOMEM;
	}

	dbg_message_diff_enable_entry = proc_create(DBG_MESSAGE_DIFF_ENABLE, 0222,
			ts->debug_info, &dbg_message_diff_enable_fops);
	if (dbg_message_diff_enable_entry == NULL) {
		NVT_ERR("create proc/touchpanel/debug_info/dbg_message_diff_enable_entry Failed!\n");
		return -ENOMEM;
	}

	dbg_finger_down_diff_entry = proc_create(DBG_FINGER_DOWN_DIFF, 0444,
			ts->debug_info, &dbg_finger_down_diff_fops);
	if (dbg_finger_down_diff_entry == NULL) {
		NVT_ERR("create proc/touchpanel/debug_info/nvt_dbg_finger_down_diff Failed!\n");
		return -ENOMEM;
	}

	dbg_status_change_diff_entry = proc_create(DBG_STATUS_CHANGE_DIFF, 0444,
			ts->debug_info, &dbg_status_change_diff_fops);
	if (dbg_status_change_diff_entry == NULL) {
		NVT_ERR("create proc/touchpanel/debug_info/nvt_dbg_status_change_diff Failed!\n");
		return -ENOMEM;
	}

	dbg_gesture_coord_record_enable_entry = proc_create(DBG_GESTURE_COORD_RECORD_ENABLE, 0222,
			ts->debug_info, &dbg_gesture_coord_record_enable_fops);
	if (dbg_gesture_coord_record_enable_entry == NULL) {
		NVT_ERR("create proc/touchpanel/debug_info/dbg_gesture_coord_record_enable Failed!\n");
		return -ENOMEM;
	}

	dbg_gesture_coord_record_entry = proc_create(DBG_GESTURE_COORD_RECORD, 0444,
			ts->debug_info, &dbg_gesture_coord_record_fops);
	if (dbg_gesture_coord_record_entry == NULL) {
		NVT_ERR("create proc/touchpanel/debug_info/dbg_gesture_coord_record Failed!\n");
		return -ENOMEM;
	}
	dbg_gesture_coord_enable_entry = proc_create(DBG_GESTURE_COORD_ENABLE, 0222,
			ts->debug_info, &dbg_gesture_coord_enable_fops);
	if (dbg_gesture_coord_enable_entry == NULL) {
		NVT_ERR("create proc/touchpanel/debug_info/dbg_gesture_coord_enable_entry Failed!\n");
		return -ENOMEM;
	}
