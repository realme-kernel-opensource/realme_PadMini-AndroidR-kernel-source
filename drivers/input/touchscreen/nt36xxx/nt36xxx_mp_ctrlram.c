...
static struct proc_dir_entry *auto_test_entry = NULL;
#if LPWG_TEST
static struct proc_dir_entry *NVT_proc_lpwg_selftest_entry = NULL;
static struct proc_dir_entry *black_screen_entry = NULL;
#endif
...
int32_t nvt_mp_proc_init(void)
{
	...
	if (NVT_proc_selftest_entry == NULL) {
		...
		if (ts->touchpanel_proc != NULL) {
			...
			auto_test_entry = proc_create("baseline_test", 0666,
					ts->touchpanel_proc, &nvt_selftest_fops);
			if (auto_test_entry == NULL) {
				NVT_ERR("create proc/touchpanel/baseline_test Failed!\n");
				return -EPERM;
			}
#if LPWG_TEST
			//proc/touchpanel/black_screen_test
			black_screen_entry = proc_create("black_screen_test", 0666,
					ts->touchpanel_proc, &nvt_lpwg_selftest_fops);
			if (black_screen_entry == NULL) {
