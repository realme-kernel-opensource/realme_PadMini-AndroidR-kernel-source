        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
	 ...
	 printk("%s ,buf :%s,suspended:%d\n",__func__,buf,pdata->suspended);
...
void himax_chip_common_resume_from_lcd(void)	
{
	//printk("%s ,buf :line = :%d\n",__func__,__LINE__);
	if(private_ts == NULL)
		return;
...
#if defined(HX_EARPHONE_MODE)
void himax_earphone_detect_func(int enable)
{
	uint8_t tmp_addr[DATA_LEN_4] = {0xE8, 0x7F, 0x00, 0x10};
	uint8_t tmp_data[DATA_LEN_4] = {0x5A, 0xA5, 0x5A, 0xA5};
	uint8_t tmp_dcls[DATA_LEN_4] = {0x00, 0x00, 0x00, 0x00};
    	int rtimes = 0;

        if (enable) {/* insert earphone */
            do {
                if (rtimes > 10) {
                    I("%s:insert earphone failed!\n", __func__);
                    break;
                }
                g_core_fp.fp_register_write(tmp_addr, 4, tmp_data, 0);
                g_core_fp.fp_register_read(tmp_addr, 4,tmp_data, 0);

                I("%s:retry times %d, current tmp_data[0, 1, 2, 3] = 0x%2X, 0x%2X, 0x%2X, 0x%2X\n", __func__,
            ...
            I("%s:insert earphone success!\n", __func__);
        } else {/* remove earphone  */
            do {
                if (rtimes > 10) {
                    I("%s:remove earphone failed!\n", __func__);
                    break;
                }

                g_core_fp.fp_register_write(tmp_addr, 4, tmp_dcls, 0);
                g_core_fp.fp_register_read(tmp_addr, 4,tmp_dcls, 0);

                I("%s:retry times %d, current tmp_dcls[0, 1, 2, 3] = 0x%2X, 0x%2X, 0x%2X, 0x%2X\n", __func__,
                           rtimes, tmp_dcls[0], tmp_dcls[1], tmp_dcls[2], tmp_dcls[3]);
                
                rtimes++;
            } while (tmp_dcls[3] != 0x00 || tmp_dcls[2] != 0x00
                     || tmp_dcls[1] != 0x00 || tmp_dcls[0] != 0x00);

            I("%s:remove earphone success!\n", __func__);
        }
}
#endif
...
static char buf[10] = {0};
	...
	snprintf(buf, sizeof(buf), "0x%2.2X", ic_data->vendor_fw_ver);
	//get_hardware_info_data(HWID_CTP_FW_INFO,buf);  // Modified by zx++ move to Himax_ic_incell_core.c
	get_hardware_info_data(HWID_CTP_DRIVER,private_ts->chip_name);
...
int himax_chip_common_init(void)
{
	...
	#if defined(CONFIG_PM) && HIMAX_PATCH_COMERR_PM
    	init_completion(&private_ts->pm_completion);
    	private_ts->pm_suspend = false;
	#endif
...
#else
	ts->ts_SMWP_wake_lock=&ts->wake_lock;//add for memory leak
...
END:
	...
	I("zx %s %d ts->suspended = %d tp_mode = %d \n",__func__,__LINE__,ts->suspended,tp_mode);
	return 0;
}
