...
void himax_headset_mode_switch(int headset_sate)
{
	...
	himax_earphone_detect_func(headset_sate);
	/*do {
		if (headset_sate ) {
...
u8 tls_tp_ver_info;
//u8 tls_vendor_info;
extern int tlsc_boot_version;
extern int tlsc_vendor_id;
int hx_get_tp_fw_info(char *buf, void *arg0) 
{
    tls_tp_ver_info = ic_data->vendor_fw_ver;
    //tls_vendor_info = tlsc_vendor_id;
    return sprintf(buf, 
        "%s-%s-%s-0x%x",
        "hsd",
        "na500",
...
#endif
int hwinfo_fw_version = 0;
void  himax_get_tp_fw_ver(void)
{
    	static char buf[10] = {0};
	memset(buf,0x0,sizeof(buf));

	printk("kaoshan %s %d fwver = 0x%x\n",__func__,__LINE__,hwinfo_fw_version);
	snprintf(buf, sizeof(buf), "%02x", hwinfo_fw_version);
...
static void himax_mcu_read_FW_ver(void)
{
	...
	switch ( data[1] & 0x07 ){
		case 1:
			I("FW Architecture : HX83102D with flash ");
			break;
		case 2:
			I("FW Architecture : with flash ");
			break;
		case 3:
			I("FW Architecture : HX83102D ZERO flash ");
			break;
		case 4:
			I("FW Architecture : zero flash ");
			break;		
		default:
			I("FW Architecture : NON ");
			break;
	}
	I("PANEL_VER : %X\n", ic_data->vendor_panel_ver);
	I("FW_VER : %X\n", ic_data->vendor_fw_ver);
	printk("Here is version");
	#ifdef CONFIG_T_PRODUCT_INFO
    {
        printk("hx_dev_info_probe %s,%d\n",__func__,__LINE__);
        FULL_PRODUCT_DEVICE_CB(ID_TP, hx_get_tp_fw_info, NULL);
    }
	#endif
	...
	hwinfo_fw_version = ic_data->vendor_touch_cfg_ver;
