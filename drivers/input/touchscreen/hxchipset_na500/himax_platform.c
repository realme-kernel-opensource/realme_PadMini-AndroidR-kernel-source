		...
		uint8_t *data, uint32_t length, uint8_t toRetry)
{
	...
	for (retry = 0; retry < toRetry; retry++) {
		...
		if (unlikely(error))
			...
			break;*/
		 for (iretry = 0; iretry < retry_timeout; iretry++) {
		        error = spi_sync(private_ts->spi, &m);
			 if (unlikely(error)){
				E("SPI read error: %d,iretry = %d\n", error,iretry);
				usleep_range(BUSY_QUERY_DELAY, BUSY_QUERY_DELAY + 50); // give up CPU
				udelay(1);
			 }else{
				break;
			}
...
void himax_reset_proc_toH(void)
{
	if (private_ts->pdata->gpio_reset >= 0) {
...
void himax_reset_proc_toL(void)
{
	if (private_ts->pdata->gpio_reset >= 0) {
	    gpio_direction_output(private_ts->pdata->gpio_reset, 0);
	    //msleep(25);
...
{
    struct himax_ts_data *himax_data = dev_get_drvdata(dev);

    printk("system enters into pm_suspend");
    himax_common_suspend(dev);
    himax_data->pm_suspend = true;
    reinit_completion(&himax_data->pm_completion);
    return 0;
}
...
static int himax_pm_resume(struct device *dev)
{
    ...
    printk("system resumes from pm_suspend");
    himax_common_resume(dev);
    himax_data->pm_suspend = false;
    complete(&himax_data->pm_completion);
    return 0;
}
