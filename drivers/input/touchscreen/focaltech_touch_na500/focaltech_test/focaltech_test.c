...
int fts_self_test(struct seq_file *s, void *v)
{
    int ret = 0;
#if 0
	printk("fts_self_test nothing to do");
#else
    struct fts_ts_data *ts_data = fts_data;
    ...
    FTS_TEST_DBG("ts_data->ic_type =  0x%x");
    //if(ts_data->ic_type == IC_TYPE_FT8201AB){
 
 	    if(black_test){
			fts_test_entry(FTS8201AB_SELF_BLACK_TEST_INI,s,v);
			touch_black_test = 0;
			FTS_TEST_INFO("black_test");
		}
		else{
			touch_black_test = 1;
			fts_test_entry(FTS8201AB_SELF_TEST_INI,s,v);
			FTS_TEST_INFO("non black_test");	
		}
