...
void fts_reset_proc_toL(void)
{
    if (fts_data->pdata->reset_gpio >= 0) {
    gpio_direction_output(fts_data->pdata->reset_gpio, 0);
    }
}

void fts_reset_proc_toH(void)
{
    if (fts_data->pdata->reset_gpio >= 0) {
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ret = FTS_proc_node_init();
	 if (ret) {
        FTS_ERROR("init proc_node_init test fail");
    }
...
//	dev_set_drvdata(&spi->dev, ts_data);
	...
	get_hardware_info_data(HWID_CTP_DRIVER,"ft8201ab");
