...
#define FTS_FW_NAME_NA500 				                 "focaltech_ts_fw.bin"
...
static int fts_get_fw_file_via_request_firmware(struct fts_upgrade *upg)
{
    ...
    snprintf(fwname, FILE_NAME_LENGTH, "%s",FTS_FW_NAME_NA500);
    ret = request_firmware(&fw, fwname, upg->ts_data->dev);
...
#if defined(CONFIG_T_PRODUCT_INFO)
static int save_fts_tp_info(char *buf, void *arg0) 
{
   	int ret = 0; 
    	u8 fwver = 0;
...
FTS_DEBUG("save_fts_tp_info,%x ",fwver);
#if defined(CONFIG_T_PROJECT_V880)
	return sprintf(buf, 
		"%s-%s-%s-v0x%x ",
		"hlt",
		"v880",
		"ft8756",
		fwver); 
#elif defined(CONFIG_T_PROJECT_V885)
	return sprintf(buf, 
		"%s-%s-%s-v0x%x ",
		"hlt",
		"v885",
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    if (ret < 0) {
        ...
        FTS_INFO("read chip id:0x%02x", chip_id);
	#if defined(CONFIG_T_PRODUCT_INFO)
	FULL_PRODUCT_DEVICE_CB(ID_TP, save_fts_tp_info, NULL);
	#endif
	//get_tp_fw_ver();
    }
}
