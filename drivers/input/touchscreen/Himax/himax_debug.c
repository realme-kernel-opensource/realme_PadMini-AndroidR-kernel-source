...
int baseline_read( struct seq_file *s );
int delta_read_sub( struct seq_file *s );
#endif
/*
...
void himax_ts_dbg_func(struct himax_ts_data *ts, int start)
{
	...
	if ((ts->debug_log_level & BIT(1)) 
		...
		|| (g_debug_level == 1)
		|| (g_debug_level == 2)){
			if((g_debug_level == 0) || (g_debug_level == 1)){
...
#if defined(PROC_NODE)	
static void himax_raw_data_read(int ts_diag_cmd)
{
	struct himax_ts_data *ts = private_ts;
	uint8_t diag_cmd = 0;
...
int baseline_read( struct seq_file *s )
{
	int x_channel = ic_data->HX_RX_NUM;
	int y_channel = ic_data->HX_TX_NUM;
	himax_raw_data_read(52); /*sram type = 5 for  test*/
...
int delta_read_sub( struct seq_file *s )
{
	int x_channel = ic_data->HX_RX_NUM;
	int y_channel = ic_data->HX_TX_NUM;
	himax_raw_data_read(51); /*sram type = 5 for  test*/
