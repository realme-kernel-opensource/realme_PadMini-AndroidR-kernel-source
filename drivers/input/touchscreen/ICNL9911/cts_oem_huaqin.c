...
#define OEM_TEST_DATA_DIR                   "/data/vendor"
...
static int black_selftest_seq_show(struct seq_file *m, void *v)
{
  ...
  //  msleep(2000);
    ...
    touch_black_test = 0;
    cts_err("end ito test touch_black_test set to 0 (touch_black_test = %d)",touch_black_test);
    return 0;
}
...
static int32_t selftest_proc_open(struct inode *inode, struct file *file)
{
    ...
    int ret;
    touch_black_test = 1;
    cts_err("start ito test touch_black_test set to 1 (touch_black_test = %d)",touch_black_test);
	...
	cts_info("Open '/proc/" OEM_SELFTEST_BLACK_FILENAME "'");
