...
int g_f_0f_updat;
	...
	#define HIMAX_PROC_REG_INFO_FILE	"register_info"
	struct proc_dir_entry *himax_proc_reg_info_file = NULL;
...
#endif
#endif
	#define HIMAX_PROC_LIMIT_ENABLE_FILE	"tp_limit_enable"
	struct proc_dir_entry *himax_proc_limit_enable_file;
	#define HIMAX_PROC_LIMIT_AREA_FILE	"tp_limit_area"
	...
	#define HIMAX_PROC_TP_DIRECTION_FILE	"tp_direction"
	struct proc_dir_entry *himax_proc_tp_direction_file = NULL;

	#define HIMAX_PROC_INCELL_PANEL_FILE	"incell_panel"
	struct proc_dir_entry *himax_proc_incell_panel_file = NULL;
	
	#define HIMAX_PROC_TX_HOP_FILE	  "tp_hop_test"
	struct proc_dir_entry *himax_proc_tx_hop_file = NULL;
	...
	struct proc_dir_entry *himax_proc_debug_level_file = NULL;
	...
	extern int baseline_read( struct seq_file *s );
	extern int delta_read_sub( struct seq_file *s );
...
static void *himax_seq_start(struct seq_file *s, loff_t *pos)
{
	if (*pos >= 1) {
		return NULL;
	}
...
static void *himax_seq_next(struct seq_file *s, void *v, loff_t *pos)
{
	return NULL;
}

static void himax_seq_stop(struct seq_file *s, void *v)
{
}
static int himax_baseline_read(struct seq_file *s, void *v) /*baseline*/
{
	size_t ret = 0;
...
static struct seq_operations himax_baseline_seq_ops = {
	.start	= himax_seq_start,
	.next	= himax_seq_next,
	.stop	= himax_seq_stop,
	.show	= himax_baseline_read,
};

static int himax_baseline_proc_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &himax_baseline_seq_ops);
};

static struct file_operations himax_proc_baseline_ops = {
	.owner = THIS_MODULE,
	.open = himax_baseline_proc_open,
	.read = seq_read,
};
...
static int himax_delta_read(struct seq_file *s, void *v) /*delta*/
{
	size_t ret = 0;

	ret = delta_read_sub( s );

	return ret;
}
static struct seq_operations himax_delta_seq_ops = {
	.start	= himax_seq_start,
	.next	= himax_seq_next,
	.stop	= himax_seq_stop,
	.show	= himax_delta_read,
};

static int himax_delta_proc_open(struct inode *inode, struct file *file)
{
	return seq_open(file, &himax_delta_seq_ops);
};

static struct file_operations himax_proc_delta_ops = {
	.owner = THIS_MODULE,
	.open = himax_delta_proc_open,
	.read = seq_read,
};
...
//himax_proc_tx_hop_op start
static ssize_t io_ctrl_read(struct file *file, char *buf,
							   size_t len, loff_t *pos)
{
	size_t count = 0;
	struct himax_ts_data *ts = private_ts;
...
static ssize_t himax_hop_test_write(struct file *file, const char *buff,/*io_ctrl*/
								size_t len, loff_t *pos)
{
...
static struct file_operations himax_proc_tx_hop_ops = {
	.owner = THIS_MODULE,
	.read = io_ctrl_read,
	.write = himax_hop_test_write,
};

//himax_proc_tx_hop_op end
								   ...
								   size_t len, loff_t *pos)
{
    ...
    if (ret >= 1) {
        ...
        switch(dbgLevel) {
            case 2:
                g_debug_level	= dbgLevel & BIT(1);
                break;
            case 1:
            case 0:
                g_debug_level = dbgLevel & BIT(0);
                break;
            default:
            g_debug_level = 0;
        };
    }
    I("%s: buf = %s, debug_level = %d\n", __func__, buf, g_debug_level);
...
/* tp_direction	Start*/
static ssize_t himax_tp_direction_write(struct file *file, const char *buff,
									size_t len, loff_t *pos)
{
	char buf_tmp[4] = {0};
	int ret = 0;
	...
	if (buf_tmp[0] == '0') {
		himax_parse_assign_cmd(fw_tp_direction_0_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_tp_direction/*0x10007f3c*/, DATA_LEN_4, buf_tmp, 0);
		I("%s: tp_direction disable .\n", __func__);
	}
	else if(buf_tmp[0] == '1')
	{
		himax_parse_assign_cmd(fw_tp_direction_1_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_tp_direction, DATA_LEN_4, buf_tmp, 0);
		I("%s: tp_direction enable .\n", __func__);
	}
	else if(buf_tmp[0] == '2')
	{
		himax_parse_assign_cmd(fw_tp_direction_2_pwd, buf_tmp, sizeof(buf_tmp));
		g_core_fp.fp_register_write(pfw_op->addr_tp_direction, DATA_LEN_4, buf_tmp, 0);
		I("%s: tp_direction enable .\n", __func__);
	}
	else {
...
static struct file_operations himax_proc_tp_direction_ops = {
	.owner = THIS_MODULE,
	.open  = simple_open,
	.read  = proc_dir_control_read,
	.write = himax_tp_direction_write,
};
/* tp_direction	End*/
...
static ssize_t hx_limit_ctrl_write(struct	file *file,	const	char __user	*buffer, size_t	count, loff_t	*ppos)
{
	...
	if(ts->limit_edge == 0)
	{
		I("%s:  send cmd 0 firmware will action in tp_direction\n", __func__);
		return count;
	}
...
const struct seq_operations coord_seq_ops =
{
...
{
    return seq_open(file, &coord_seq_ops);
}
...
//HIMAX_PROC_REG_INFO START
static ssize_t himax_reg_read_read(struct file *file, char *buf, size_t len, loff_t *pos)
{
	int ret = 0;
	uint16_t loop_i;
	uint8_t data[128];
...
static ssize_t himax_reg_read_write(struct file *file, const char *buff,
		size_t len, loff_t *pos)
{
	char buf[80] = {0};
	char buf_tmp[16];
...
static struct file_operations himax_proc_reg_read_ops = {
	.owner = THIS_MODULE,
	.read = himax_reg_read_read,
	.write = himax_reg_read_write,
};
//HIMAX_PROC_REG_INFO END
...
int proc_node_init(void)
{
	...
	himax_proc_reg_info_file = proc_create(HIMAX_PROC_REG_INFO_FILE, (S_IWUSR | S_IRUGO),
								  himax_proc_touchpanel_dir, &himax_proc_reg_read_ops);
	if (himax_proc_reg_info_file == NULL) {
		E(" %s: proc reg_info file create failed!\n", __func__);
		goto fail_proc_0;
	}
	...
	himax_proc_tp_direction_file = proc_create(HIMAX_PROC_TP_DIRECTION_FILE, (S_IWUSR | S_IRUGO | S_IWUGO),
								  himax_proc_touchpanel_dir, &himax_proc_tp_direction_ops);
	if (himax_proc_tp_direction_file == NULL) {
		E(" %s: proc tp_direction file create failed!\n", __func__);
		goto fail_proc_7;
	}
	...
	himax_proc_tx_hop_file = proc_create(HIMAX_PROC_TX_HOP_FILE, (S_IWUSR | S_IRUGO | S_IWUGO),
								  himax_proc_touchpanel_dir, &himax_proc_tx_hop_ops);
	if (himax_proc_tx_hop_file == NULL) {
	...
	himax_proc_debug_level_file = proc_create(HIMAX_PROC_DEBUG_LEVEL_FILE, (S_IWUSR | S_IRUGO | S_IWUGO),
								  himax_proc_touchpanel_dir, &himax_proc_debug_level_ops);
...
fail_proc_dbg_0:
	remove_proc_entry(HIMAX_PROC_DEBUG_LEVEL_FILE, himax_proc_touchpanel_dir);
fail_proc_13:
...
fail_proc_10:
	remove_proc_entry(HIMAX_PROC_TX_HOP_FILE, himax_proc_touchpanel_dir);	
fail_proc_9:
	remove_proc_entry(HIMAX_PROC_INCELL_PANEL_FILE, himax_proc_touchpanel_dir);	
fail_proc_8:
	remove_proc_entry(HIMAX_PROC_TP_DIRECTION_FILE, himax_proc_touchpanel_dir);	
fail_proc_7:
...
fail_proc_1:
	remove_proc_entry(HIMAX_PROC_REG_INFO_FILE, himax_proc_touchpanel_dir);
fail_proc_0:	
...
#if defined(MD_PROC_NODE) /* gesture parsing*/
...
static void tp_fw_update_work(struct work_struct *work)
{
	...
	if ( g_core_fp.fp_firmware_update_0f != NULL) {
		...
		do {
             ...
             if(tp_hx83112a_inx)
             {
                 ret = request_firmware(&fw_entry, BOOT_UPGRADE_FWNAME_INX, ts->dev);
                 if (ret < 0) {
                	 I("%s: fw request fail \n", __func__);
                 }else {
                	 break;
...
#if defined(MD_PROC_NODE)	
	if (proc_node_init()) {
		E(" %s:proc_node_init failed!\n", __func__);
		goto err_creat_proc_failed;
	}	
...
void himax_edge_mode_set(char *buf,int cnt)
{
	...
	else if(buf_tmp[0] == '2')
	{
		...
		printk("%s: tp_direction wrong parameter .\n", __func__);
		return ;
	}


	g_core_fp.fp_register_read(pfw_op->addr_tp_direction,DATA_LEN_4, tmp_tmp, 0);
