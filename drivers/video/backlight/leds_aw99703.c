...
unsigned int tn_brightness_map(struct aw99703_data *drvdata,unsigned int level);
const char *blbootmode;
...
//	pr_info("%s brt_val is %d\n", __func__, brt_val);
	...
	if (drvdata->inited == false) {
		 aw99703_backlight_init(drvdata); //remove ,use uboot init
	...
	if (brt_val > 0) {
		if(blbootmode!=NULL&&!strcmp(blbootmode, "silent")&&drvdata->silent_reboot_complete!=1){
				pr_err("aw99703_set_brightness input brt_val=%d,silent reboot mode so return\n",brt_val);
				return 0;
		}

	if(brt_val <drvdata->levels[(drvdata->num-1)])
	brt_val =tn_brightness_map(drvdata,brt_val);
...
static int aw99703_bl_update_status_cabc(struct backlight_device *bl_dev)
{
		struct aw99703_data *drvdata = bl_get_data(bl_dev);
		int brt;

		brt = drvdata->sprd_bl.cabc_level;
		//pr_info("cabc brightness level: %d\n", brt);
		if(brt<=13){
			brt=13;
		}
		/*
...
static const struct backlight_ops aw99703_bl_ops = {
		...
		.update_status_cabc =aw99703_bl_update_status_cabc,
};
#endif
	...
	struct aw99703_data *drvdata)
{
	...
	prop = of_find_property(np, "brightness-levels", &length);
	if (prop) {
		drvdata->num = length / sizeof(u32);

		/* read brightness levels from DT property */
		if (drvdata->num > 0) {
			size_t size = sizeof(u32) * drvdata->num;

			drvdata->levels = devm_kzalloc(dev, size, GFP_KERNEL);
			if (!drvdata->levels)
				return ;

			rc = of_property_read_u32_array(np,
							"brightness-levels",
							drvdata->levels, drvdata->num);
		}
	}
}
...
i=(level*(drvdata->num-1))/drvdata->levels[drvdata->num-1];
real_level=drvdata->levels[i]+((level*(drvdata->num-1)-i*drvdata->levels[(drvdata->num-1)])*(drvdata->levels[i+1]-drvdata->levels[i])+drvdata->levels[(drvdata->num-1)]/2)/drvdata->levels[(drvdata->num-1)];

//(level-i*drvdata->levels[255]/255)

//drvdata->levels[i]

/*
for(i=0;i<drvdata->num;i++){
	if(drvdata->levels[i]>=level)
		break;
	}
real_level=i*(drvdata->levels[255]-drvdata->levels[0])/255
+((level-drvdata->levels[i-1])*(drvdata->levels[255]-drvdata->levels[0])+(drvdata->levels[i]-drvdata->levels[i-1])*128)/((drvdata->levels[i]-drvdata->levels[i-1])*255);
*/
//	pr_err("tn_brightness_map input leave=%d,real_level=%d\n",level,real_level);
...
/******************************************************
 *
 * sys group attribute: silent_reboot_complete
 *
 ******************************************************/
static ssize_t silent_reboot_complete_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw99703_data *aw99703 = dev_get_drvdata(dev);
	u32 value;
	...
	aw99703->silent_reboot_complete = value;
	
	return count;
}

static ssize_t silent_reboot_complete_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct aw99703_data *aw99703 = dev_get_drvdata(dev);
	return snprintf(buf, PAGE_SIZE, "0x%02x\n",
			aw99703->silent_reboot_complete);

}

static DEVICE_ATTR_RW(silent_reboot_complete);

static struct attribute *aw99703_attributes[] = {
	&dev_attr_reg.attr,
	&dev_attr_silent_reboot_complete.attr,
	NULL
};
			...
			const struct i2c_device_id *id)
{
	...
	drvdata->enable = true;
	drvdata->inited = true;//uboot inited
	...
	props.brightness = 2047;//MAX_BRIGHTNESS;
	props.max_brightness = 2047;//MAX_BRIGHTNESS;
	drvdata->sprd_bl.cabc_en=1;
	drvdata->sprd_bl.max_level=2047;
	drvdata->sprd_bl.min_level=1;
	bl_dev = backlight_device_register("sprd_backlight", &client->dev,
