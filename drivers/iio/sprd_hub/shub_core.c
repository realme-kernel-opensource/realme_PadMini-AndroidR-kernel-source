...
#define PS_CALIB_TOLERANCE			10 // hjddbg
#define PS_CALIB_MIN_DIFF_NEAR		20 // hjddbg
#define PS_CALIB_MIN_DIFF_FAR		10 // hjddbg

//static int	si_dbg_cm4_op    = 0; // hjddbg
static u16	sgu16_ps_noise   = 0; // hjddbg
static u16	sgu16_ps_near    = 0; // hjddbg
static u16	sgu16_ps_far     = 0; // hjddbg
static u16	sgu16_ps_last_op = 0; // hjddbg,  1=noise ; 2=near ; 3=far;
...
static int check_proximity_cali_data(void *cali_data)
{
	...
	pr_err("hjddbg, check_proximity_cali_data abs(noise)=%d, abs(near)=%d, abs(far)=%d"
		, abs(prox_cali.ground_noise - sgu16_ps_noise)
		, abs(prox_cali.high_threshold - sgu16_ps_near)
		, abs(prox_cali.low_threshold - sgu16_ps_far) );
...
/* prox sensor factory manual calibration */
	if ((prox_cali.cali_flag & 0x06) == 0x06 &&
		(prox_cali.high_threshold - prox_cali.low_threshold) < 20) {
		dev_err(&sensor->sensor_pdev->dev,
			"prox sensor cali failed! the high_thrd < low_thrd!\n");
		printk("factory calibration fail\n");
		return -EINVAL;
	}
	if ((prox_cali.cali_flag & 0x07) == 0x07 && 3 == sgu16_ps_last_op)
	{
		pr_err("hjddbg, noise=%d(g=%d), high=%d(%d), low=%d(%d)", prox_cali.ground_noise, sgu16_ps_noise
				, prox_cali.high_threshold, sgu16_ps_near, prox_cali.low_threshold, sgu16_ps_far);

		if (   prox_cali.ground_noise > prox_cali.low_threshold
			|| prox_cali.low_threshold > prox_cali.high_threshold
			|| abs(prox_cali.ground_noise - sgu16_ps_noise) > PS_CALIB_TOLERANCE 
			|| abs(prox_cali.low_threshold - sgu16_ps_far) > PS_CALIB_TOLERANCE 
			|| abs(prox_cali.high_threshold - sgu16_ps_near) > PS_CALIB_TOLERANCE
			|| abs(prox_cali.high_threshold - prox_cali.ground_noise) < PS_CALIB_MIN_DIFF_NEAR
			|| abs(prox_cali.low_threshold - prox_cali.ground_noise) < PS_CALIB_MIN_DIFF_FAR
		) {
			pr_err("hjddbg, double check data, not available! ");
			return -EINVAL;
		}
	}
...
static void shub_save_calibration_data(struct work_struct *work)
{
    ...
    //Donghua.Gan@ODM_HQ.AD.EngineerMode,2021/2/24, Modify permission for file by read calibrated data.
				  ...
				  const char *buf, size_t count)
{
	...
	if (sensor->mcu_mode <= SHUB_OPDOWNLOAD) {
		pr_err("hjddbg, light_sensor_calibrator_store, mcu_mode == SHUB_BOOT!\n");
		return -EINVAL;
	}

	len = sscanf(buf, "%d %d\n", &sensor->cal_cmd, &sensor->cal_id);
	if (len < 2) {
        pr_err("hjddbg, light_sensor_calibrator_store, sscanf=%d \n", len);
		return -EINVAL;
	}
	pr_err("hjddbg, light_sensor_calibrator_store, id:%d,type:%d\n", sensor->cal_cmd, sensor->cal_id);
	if (sensor->cal_cmd != CALIB_DATA_WRITE ||
		sensor->cal_id != SENSOR_TYPE_LIGHT) {
		pr_err("hjddbg, light_sensor_calibrator_store, light sensor cali cmd error\n");
		return -EINVAL;
	}

	err = set_als_calib_cmd(sensor, sensor->cal_cmd, sensor->cal_id);
	if (err < 0)
		pr_err("hjddbg, light_sensor_calibrator_store, light sensor cali Fail! set_als_calib_cmd=%d \n", err);
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	err = kernel_read(pfile, raw_cali_data,
			      ...
			      &pfile->f_pos);
	if (err < 0) {
		pr_err("hjddbg, light_sensor_calibrator_show, Error: file read failed\n");
		kfree(raw_cali_data);
		filp_close(pfile, NULL);
		return err;
	}
	...
	pr_err("hjddbg, light_sensor_calibrator_show, cali_data_coef = %d\n", cali_data_coef);
	// if (cali_data_coef < 0) // hjddbg
	if (cali_data_coef < 100 || cali_data_coef > 1000000) // hjddbg, [0.01, 100]==OK
		status = CALIB_STATUS_FAIL;
	else
		status = CALIB_STATUS_PASS;
...
#if 1
	if (0 == sensor->cal_cmd) { // calibrating cmd 
		sgu16_ps_last_op = 0;
	}
	
	// AUTO_SET_CMD        = "0 8 1"; // auto calibrating
	// MANUAL_NEAR_SET_CMD = "0 8 5"; // manual near calibrating
	// MANUAL_FAR_SET_CMD  = "0 8 6"; // manual far calibrating
	// TEST_GET_RESULT     = "1 8 1"; // get result of Calibration
	// TEST_SAVE_RESULT    = "3 8 1"; // save the result
	if (0 == sensor->cal_cmd && 8 == sensor->cal_id) { // p-sensor
		u16  data = 0;
		
	    pr_err("hjddbg, %s p-sensor calib type=%d \n", __func__, sensor->cal_type);
		err = shub_sipc_read(sensor, SHUB_GET_PROXIMITY_RAWDATA_SUBTYPE, (u8*)&data, sizeof(data));
		if (err < 0) {
			pr_err("hjddbg, cali_cmd get ps rawdata failed!\n");
			return err;
		}

		if (1 == sensor->cal_type) { // auto
			sgu16_ps_noise = data;
			sgu16_ps_last_op = 1;
			// hjdpending
			// sgu16_ps_near = 0;
		...
		else if (5 == sensor->cal_type) { // near
			sgu16_ps_near = data;
			sgu16_ps_last_op = 2;
		}
		else if (6 == sensor->cal_type) { // far
			sgu16_ps_far = data;
			sgu16_ps_last_op = 3;
		}		
	}
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (sensor->cal_cmd == CALIB_CHECK_STATUS) // hjddbg, hjdpending, "1 8 1"
	{
		char  result = sensor->calibrated_data[0];

		if (0 != sgu16_ps_last_op)
		{
			int   ps_nosie = 0;
			int   ps_near  = 0;
			int   ps_far   = 0;
			...
			// TEST_SAVE_RESULT    = "3 8 1"; // save the result
			err = set_calib_cmd(sensor, 3, 8, 1, 0); // get the calibration result, but not save
			msleep(100);		
			err = shub_sipc_read(sensor, SHUB_GET_CALIBRATION_DATA_SUBTYPE,
							     sensor->calibrated_data, CALIBRATION_DATA_LENGTH);

			ps_nosie = *((int *)(sensor->calibrated_data));
			ps_near  = *((int *)(sensor->calibrated_data + 4));
			ps_far   = *((int *)(sensor->calibrated_data + 8));

			pr_err("hjddbg, PS result=%d, now noise=%d(o=%d), near=%d(o=%d), far=%d(o=%d) \n"
					, result, ps_nosie, sgu16_ps_noise, ps_near, sgu16_ps_near, ps_far, sgu16_ps_far);

			// hjdpending
			result = 0;
			
			if ( 1 == sgu16_ps_last_op && abs(sgu16_ps_noise - ps_nosie) < PS_CALIB_TOLERANCE)
			{
				pr_err("hjddbg, noise OK, abs(noise-noise)=%d \n", abs(sgu16_ps_noise - ps_nosie));
				result = 2;
			}
			
			if ( 2 == sgu16_ps_last_op && abs(sgu16_ps_near - ps_near) < PS_CALIB_TOLERANCE
				&& abs(ps_near - sgu16_ps_noise) > PS_CALIB_MIN_DIFF_NEAR)
			{
				pr_err("hjddbg, near OK, abs(near-noise)=%d \n", abs(ps_near - sgu16_ps_noise));
				result = 2;
			}
			
			if ( 3 == sgu16_ps_last_op && abs(sgu16_ps_far - ps_far) < PS_CALIB_TOLERANCE
				&& abs(ps_far - sgu16_ps_noise) > PS_CALIB_MIN_DIFF_FAR)
			{
				pr_err("hjddbg, far OK, abs(far-noise)=%d \n", abs(ps_far - sgu16_ps_noise));
				result = 2;
			}
		}
