...
#define NICO_USB20_TRIMMING_BITS_REGION_MASK   	GENMASK(25, 21)
#define NICO_USB20_TRIMMING_BITS_REGION_SET		(0x12  << 21)

#define NICKY_USB20_TRIMMING_BITS_REGION_MASK   0xfffffff
#define NICKY_USB20_TRIMMING_BITS_REGION_SET	0x483d1c0

#define FREYA_USB20_TRIMMING_BITS_REGION_SET	0x603d1c0
...
#ifdef CONFIG_USE_EXTERNAL_BC12_CHECK
extern bool sgm4154x_charger_detect(enum usb_charger_type *ch_ty, bool skip_force);
extern enum usb_charger_type sgm4154x_charger_redetect(void);
#else
enum usb_charger_type sc27xx_charger_redetect(void)
{
...
#ifndef CONFIG_USE_EXTERNAL_BC12_CHECK
...
void eye_pattern_set()
{
    void __iomem *reg_base = ioremap_nocache(0x64570010, 4);
    uint32_t value;
    value = readl_relaxed(reg_base);

	if(prj_name == 1 || prj_name == 3){//nico
		value &= ~NICO_USB20_TRIMMING_BITS_REGION_MASK;
		value |= NICO_USB20_TRIMMING_BITS_REGION_SET;
	}else if(prj_name == 2){//nicky
		value &= ~NICKY_USB20_TRIMMING_BITS_REGION_MASK;
		value |= NICKY_USB20_TRIMMING_BITS_REGION_SET;
	}else if(prj_name == 20){//freya
		value = FREYA_USB20_TRIMMING_BITS_REGION_SET;
	}else{
		value = DEFAULT_HOST_EYE_PATTERN;
	}
...
#ifdef CONFIG_USE_EXTERNAL_BC12_CHECK
#define BC12_CHECK_TIME         2
static volatile unsigned char in_det_cnt = 0;
static enum usb_charger_type a_ch_type[4] = {0, 0};
void set_init_in_det_cnt(void)
{
	in_det_cnt = 0;
}
#endif
...
{
#ifdef CONFIG_USE_EXTERNAL_BC12_CHECK
    enum usb_charger_type ch_type;
	int cnt;
	
	for(cnt = 0; cnt < 60; cnt ++) {
		if (in_det_cnt >= BC12_CHECK_TIME)break;
	    if (sgm4154x_charger_detect(&ch_type, (in_det_cnt == 0))) {
			if (in_det_cnt <= BC12_CHECK_TIME) {
				a_ch_type[in_det_cnt] = ch_type;
				in_det_cnt ++;
			}
			break;
		}
	...
	dev_info(x->dev, "[%s] ch_type1[%d] cnt[%d] in_det_cnt[%d]\n", __func__, a_ch_type[0], cnt, in_det_cnt);
	...
	dev_info(x->dev, "[%s] ch_type2[%d] cnt[%d] in_det_cnt[%d]\n", __func__, a_ch_type[1], cnt, in_det_cnt);
	
	if (a_ch_type[0] != UNKNOWN_TYPE)return a_ch_type[0];
	else return a_ch_type[1];
#else
	struct sprd_hsphy *phy = container_of(x, struct sprd_hsphy, phy);
...
static enum usb_charger_type sprd_hsphy_retry_charger_detect(struct usb_phy *x)
{
...
#ifdef CONFIG_USE_EXTERNAL_BC12_CHECK
	{
		...
		dev_info(x->dev, "[sprd_hsphy_retry_charger_detect]correct type is %x\n", type);
