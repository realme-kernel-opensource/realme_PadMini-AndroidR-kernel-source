...
#define SC27XX_CC_OPEN			0x5000
#define SC27XX_CC_DEFAULT		0x0000
#define SC27XX_CC_CFG_MASK		0xF000
...
int sc27xx_typec_set_cc_open(struct sc27xx_typec *sc)
{
	int ret;
	u32 val;

	printk("sc27xx_typec_set_cc_open zq read\n");
	ret = regmap_read(sc->regmap, sc->base + SC27XX_SW_CFG, &val);
	if (ret)
		return ret;
	printk("sc27xx_typec_set_cc_open zq before write,val=0x%x\n",val);

	val &= ~SC27XX_CC_CFG_MASK;
	val |= SC27XX_CC_OPEN;

	ret = regmap_write(sc->regmap, sc->base + SC27XX_SW_CFG, val);
	if (ret){
		printk("sc27xx typec set cc open fail\n");
		return ret;
	}
	printk("sc27xx_typec_set_cc_open zq after write,val=0x%x\n",val);

	return 0;

}
EXPORT_SYMBOL_GPL(sc27xx_typec_set_cc_open);

int sc27xx_typec_set_cc_default(struct sc27xx_typec *sc)
{
	int ret;
	u32 val;

	printk("sc27xx_typec_set_cc_default zq read\n");
	ret = regmap_read(sc->regmap, sc->base + SC27XX_SW_CFG, &val);
	if (ret)
		return ret;
	printk("sc27xx_typec_set_cc_default zq before write,val=0x%x\n",val);

	val &= ~SC27XX_CC_CFG_MASK;
	val |= SC27XX_CC_DEFAULT;

	ret = regmap_write(sc->regmap, sc->base + SC27XX_SW_CFG, val);
	if (ret){
		printk("sc27xx typec set cc default fail\n");
		return ret;
	}
	printk("sc27xx_typec_set_cc_default zq after write,val=0x%x\n",val);

	return 0;

}
EXPORT_SYMBOL_GPL(sc27xx_typec_set_cc_default);
