...
extern int prj_name ;	//1: NICO 2: NICKY 3:NICKY-A 20:FREYA
...
extern void set_init_in_det_cnt(void);
#endif
	...
	if (event) {
		...
		if (glue->vbus_active == 1 || glue->dr_mode == USB_DR_MODE_HOST) {
			...
			if (prj_name == 2 || prj_name == 3 || prj_name == 20) {
				...
				dev_info(glue->dev, "%s %d usb switch 0\n", __func__, __LINE__);
			}
#endif
			dev_info(glue->dev,
		...
		if (prj_name == 2 || prj_name == 3|| prj_name == 20)
		{
			...
			dev_info(glue->dev, "%s %d usb switch 1\n", __func__, __LINE__);
		}		
		#else
		if (prj_name == 2 || prj_name == 3|| prj_name == 20)
		{
			dev_info(glue->dev, "%s usb switch 0\n", __func__);
			gpiod_set_value_cansleep(glue->sw, 0);
		}
		#endif
		dev_info(glue->dev,
...
#endif
//extern bool sgm4154x_check_qc20_work(void);
static bool musb_sprd_is_connect_host(struct sprd_glue *glue)
{
	...
	dev_info(glue->dev, "%s prj_name=%d  type=%d\n", __func__, prj_name, (int)type);
	if (type == SDP_TYPE || type == CDP_TYPE)
		return true;
    if (prj_name == 2 || prj_name == 3|| prj_name == 20) {
        if (type == DCP_TYPE || type == UNKNOWN_TYPE) {
		    gpiod_set_value_cansleep(glue->sw, 1);	
			dev_info(glue->dev, "%s usb switch 1\n", __func__);	
		    //sgm4154x_check_qc20_work();
		}
    }
#endif	
...
dev_err(glue->dev, "!!!!prj_name=[%d] bbat_mode=[%d]\n", prj_name, bbat_mode);
	if (prj_name == 2 || prj_name == 3|| prj_name == 20) {
		glue->sw = devm_gpiod_get_index(&pdev->dev, "usb-sw", 0, GPIOD_OUT_LOW);
		if (IS_ERR(glue->sw)) {
			dev_err(glue->dev, "!!!!failed to get enable gpio\n");
			//return PTR_ERR(info->gpiod);
		}
		else {
			dev_err(glue->dev, "!!!!ok to get enable gpio\n");
		}
	...
	/*liuting@ODM.HQ.BSP.CHG 2021/11/10 add for slow usb*/
    musb = platform_get_drvdata(glue->musb);
	if(bbat_mode){
	    dev_err(glue->dev, "bbat_mode : for slow usb\n");
