					 ...
					 u32 input_vol)
{
	...
	unsigned int snk_uw;
	dev_info(info->dev, "sc2730_fchg_pd_adjust_voltage input_vol=%d\n", input_vol);
				 ...
				 unsigned long event, void *data)
{
	...
	dev_err(info->dev, "eddyhua psy->desc->name %s %s %d\n", psy->desc->name, __func__, __LINE__);

	if (strcmp(psy->desc->name, "sgm4154x_charger") != 0) {
		dev_info(info->dev, "eddyhua %s %d\n", __func__, __LINE__);
		goto out;
	}

	dev_err(info->dev, "eddyhua %s %d\n", __func__, __LINE__);
...
static int g_pd_type = 0;
bool check_pd_type(void)
{
    if ( (g_pd_type == POWER_SUPPLY_USB_TYPE_PD) || (g_pd_type == POWER_SUPPLY_USB_TYPE_PD_DRP) || (g_pd_type == POWER_SUPPLY_USB_TYPE_PD_PPS) )
	{
		printk("[cstest] %s : line=%d g_pd_type=%d\n", __func__, __LINE__, g_pd_type);
		return true;
	}
	else
	{
