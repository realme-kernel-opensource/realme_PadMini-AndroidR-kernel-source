...
#define SGM41512_FAST_CHG_VOL_MAX		10000000
...
static int sgm41512_charger_start_charge(struct sgm41512_charger_info *info)
{
	...
	ret = sgm41512_update_bits(info, SGM41512_REG_01, REG01_CHG_CONFIG_MASK,
				       ...
				       u32 cur)
{
	u8 reg_val;
	int ret;
dev_info(info->dev, "sgm41512_charger_set_current :%d\n", cur);
					     ...
					     u32 limit_cur)
{
	u8 reg_val;
	int ret;	
dev_info(info->dev, "sgm41512_charger_set_limit_current :%d\n", limit_cur);
					    ...
					    u32 val)
{
	...
	if (val == CM_FAST_CHARGE_ENABLE_CMD) {
		...
		if (info->role == SGM41512_ROLE_SLAVE) {
			...
			#else
//			limit_cur = info->cur.fchg_limit *2/5;
//			cur = info->cur.fchg_cur *2/5;
			limit_cur = info->cur.fchg_limit * 96 / 300;
			cur = info->cur.fchg_cur * 96 / 300;
			#endif
		}
