...
#define REG0D_JEITA_EN_MASK      GENMASK(0, 0)
...
#define SGM4154x_FAST_CHG_VOL_MAX		10000000
#define SGM4154x_NORMAL_CHG_VOL_MAX		5800000
...
{
	u8 reg_val;
dev_err(info->dev, "sgm4154x_charger_set_termina_vol:%d\n", vol);

    // for 4.128v  Vcv
	if (vol == 4128) {
		vol = 4144;		
		sgm4154x_update_bits(info, SGM4154x_REG_0F, REG0F_VREG_FT_MASK,
				   3 << 6);
	}
	else {
		sgm4154x_update_bits(info, SGM4154x_REG_0F, REG0F_VREG_FT_MASK,
				       ...
				       u32 cur)
{
	u8 reg_val;
	int ret;
dev_info(info->dev, "sgm4154x_charger_set_current :%d\n", cur);
					     ...
					     u32 limit_cur)
{
	u8 reg_val;
	int ret;
dev_info(info->dev, "sgm4154x_charger_set_limit_current :%d\n", limit_cur);
...
bool sgm4154x_is_hvdcp(struct sgm4154x_charger_info *info)
{
	...
	dev_err(info->dev, "[sgm4154x_is_hvdcp]finn vbus is %d\n", val.intval);
...
static int sgm4154x_qc_adjust_voltage(struct sgm4154x_charger_info *info, u32 value)
{
	...
	if (value == SGM4154x_NORMAL_CHARGER_VOLTAGE) {
		...
		dev_info(info->dev, "sgm4154x_qc_adjust_voltage 1 adjust 5v vbus=%d\n", vbus);
		return  ret;
		...
		for (i = 0; i < 10; i++) {
			...
			#else
			vbus = sgm4154x_charger_get_vbus(info);
			dev_info(info->dev, "sgm4154x_qc_adjust_voltage 2 vbus=%d [cnt=%d]\n", vbus, i);
		    if (vbus < 3000000) {
			    return -EINVAL;
		    }
			if (vbus > 7000 * 1000)
		...
		if (!sgm4154x_is_hvdcp(info))
		{
			do {
				...
				vbus = sgm4154x_charger_get_vbus(info);
				dev_info(info->dev, "sgm4154x_qc_adjust_voltage 3 vbus=%d\n", vbus);
		        if (vbus < 3000000) {
			        return -EINVAL;
		        }				
...
extern bool check_pd_type(void);
static void sgm4154x_charger_qc_detection_work(struct work_struct *work)
{
	...
	dev_info(info->dev, "fchg %s qc start detect: check_pd_type=%d chg_type=%d\n", __func__, check_pd_type(), info->usb_phy->chg_type);
	if(check_pd_type())
	{
		dev_info(info->dev, "fchg %s find PD return .. %d\n", __func__, __LINE__);
		return;
	}
	...
	while (!sgm4154x_is_hvdcp(info) &&(info->usb_phy->chg_type == DCP_TYPE)) {
		...
		if (!flag) {
			...
			if(check_pd_type() || (vbus < 3000000))
	        {
		        dev_info(info->dev, "fchg %s find PD or others return .. %d\n", __func__, __LINE__);
		        return;
	        }
...
static int sgm4154x_vdmp_bat_track_set(struct sgm4154x_charger_info *info, u32 vol)
{
	u8 reg_val;
    // DISABLE JEITA
    sgm4154x_update_bits(info, SGM4154x_REG_0D,
						REG0D_JEITA_EN_MASK,
						0);

	sgm4154x_update_bits(info, SGM4154x_REG_07,
						REG07_BATFET_RST_EN_MASK,
						0 << REG07_BATFET_RST_EN_SHIFT);
...
static int sgm4154x_charger_hw_init(struct sgm4154x_charger_info *info)
{
	...
	if (ret) {
		...
		dev_warn(info->dev, "battery information is supplied\n");
		...
		ret = sgm4154x_set_prechrg_curr(info,300);
		if (ret)
...
static int sgm4154x_charger_start_charge(struct sgm4154x_charger_info *info)
{
	...
	ret = sgm4154x_update_bits(info, SGM4154x_REG_01, REG01_CHG_CONFIG_MASK,
				  REG01_CHG_ENABLE << REG01_CHG_CONFIG_SHIFT);
	if (ret)
		dev_err(info->dev, "Failed to enable sgm4154x charge\n");
...
static void sgm4154x_charger_stop_charge(struct sgm4154x_charger_info *info)
{
	...
	if (ret)
		dev_err(info->dev, "Failed to disable sgm4154x charge\n");
					    ...
					    u32 val)
{
	...
	if (val == CM_FAST_CHARGE_ENABLE_CMD) {
		...
		#else
//		limit_cur = info->cur.fchg_limit *3/5;
//		cur = info->cur.fchg_cur *3/5;
		limit_cur = info->cur.fchg_limit * 204 / 300;
		cur = info->cur.fchg_cur * 204 / 300;
		#endif
					    ...
					    const union power_supply_propval *val)
{
	...
	dev_info(info->dev, "enter %s,psp = %d\n",__func__,psp);
	mutex_lock(&info->lock);
...
static volatile unsigned char in_det_flag = false;
bool sgm4154x_charger_detect(enum usb_charger_type *ch_ty, bool skip_force)
{
	int type = 0;
	enum usb_charger_type chr_type = UNKNOWN_TYPE;
	u8 chrg_stat = UNKNOWN_TYPE;
	int i = 0;
	...
	if (in_det_flag)return false;
	in_det_flag = true;

//dev_err(pinfo->dev, "finn chrg_stat skip_force = %d\n", skip_force);
    if (false == skip_force) {
		sc27xx_force_dpdm();
	    mdelay(50);
...
enum usb_charger_type sgm4154x_charger_redetect(void)
{
	...
	do {
		sgm4154x_charger_detect(&type, false);
