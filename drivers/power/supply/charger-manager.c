...
#define CM_FAST_CHARGE_ENABLE_CURRENT		900000 // 1200000
...
#define CM_FAST_CHARGE_ENABLE_BATTERY_VOLTAGE_MAX   4500000
#define CM_FAST_CHARGE_ENABLE_TEMP_MAX      420
#define CM_FAST_CHARGE_ENABLE_TEMP_MIN      -100

#define CM_HIGH_TEMP_STATUS      6
#define CM_HIGH_TEMP_STATUS_CV      4070000

#define USBTEMP_CC_OPEN_INTERVAL   (300*1000) /*5 mins*/


//#define TEST_FIX_BAT_LEVEL
extern bool g_bat_fix_temp;
...
extern int sc27xx_typec_set_cc_open(struct sc27xx_typec *sc);
extern int sc27xx_typec_set_cc_default(struct sc27xx_typec *sc);
extern int sc27xx_typec_set_sink(struct sc27xx_typec *sc);
extern int sc27xx_typec_set_enable(struct sc27xx_typec *sc);
extern int sgm4154x_enable_shipmode(struct sgm4154x_charger_info *info,bool en);
extern int sgm4154x_set_prechrg_curr(struct sgm4154x_charger_info *info,int pre_current);
extern int sgm41512_set_prechrg_curr(struct sgm41512_charger_info *info,int pre_current);
...
static int otg_switch_state = 0xff;
...
int ship_mode = 0;
int g_cool_down = 0;
static int charger_full_count = 0;
...
bool ccali_mode=false;
static int g_batt_jeita_temp = 0;
...
static int __init get_prj_name_setup(char *str)
{
	...
	if (!strcmp(str, "21660") || !strcmp(str, "21661") || !strcmp(str, "21662") || !strcmp(str, "216DA") || !strcmp(str, "216DB") || !strcmp(str, "21735"))
		prj_name = 1;//NICO
	else if(!strcmp(str, "2171A")|| !strcmp(str, "2171C")|| !strcmp(str, "2171B")|| !strcmp(str, "2171D")|| !strcmp(str, "2171E") || !strcmp(str, "21724") )
		prj_name = 2;//NICKY
	else if(!strcmp(str, "2171F")|| !strcmp(str, "21721")|| !strcmp(str, "21720")|| !strcmp(str, "21722")|| !strcmp(str, "21723"))
		prj_name = 3;//NICKY-A
	else if( !strcmp(str, "2172C")|| !strcmp(str, "2172D")|| !strcmp(str, "2172E") )
		prj_name = 20;//FREYA
...
//	cur[1] = target_cur*4/10;
	cur[0] = target_cur * 204 / 300;
	cur[1] = target_cur *  96 / 300;
...
printk("%s 1\n", __func__);
	...
	if (!psy) {
		...
		dev_err(cm->dev, "%s find power supply \"%s\"\n", __func__,
			desc->psy_charger_stat[0]);
	}
...
static bool cm_is_reach_fchg_threshold(struct charger_manager *cm)
{
	...
	if (cm->desc->cap <= CM_FAST_CHARGE_ENABLE_CAP)
		fast_charge_enable_count++;

	ret = true;
	if (fast_charge_enable_count < CM_FAST_CHARGE_ENABLE_COUNT || g_cool_down == 1)
		ret = false;

	dev_info(cm->dev, "cm_is_reach_fchg_threshold: batt_uA = %d, batt_uV = %d, cur_temp = %d, cap = %d, cool_down = %d, ret = %d\n",
			batt_uA, batt_uV, cur_temp, cm->desc->cap, g_cool_down, ret);
	return ret;
}
...
//	dev_err(cm->dev, "eddyhua %s %d\n", __func__, __LINE__);
	...
	//    dev_info(cm->dev, "eddyhua %s %d adapter_max_vbus=%d fast_charger_type=%d\n", 
	//	    __func__, __LINE__, adapter_max_vbus, cm->desc->fast_charger_type);

		if (adapter_max_vbus < CM_FAST_CHARGE_VOLTAGE_9V)return 0;
	}
...
//	dev_err(cm->dev, "eddyhua %s %d adapter_max_vbus = %d\n", __func__, __LINE__, adapter_max_vbus);
...
//	dev_err(cm->dev, " fast charge check success %s %d\n", __func__, __LINE__);
...
{
	int batt_uV = 0, batt_uA = 0, ret = 0, cur_temp = 0;
    ...
    dev_info(cm->dev, "cm_fast_charge_disable_check: batt_uA = %d, batt_uV = %d, cur_temp = %d, cap = %d, cool_down = %d\n",
...
static int try_fast_charger_enable(struct charger_manager *cm, bool enable)
{
	...
	dev_err(cm->dev, "eddyhua %s cm->desc->fast_charger_type = %d\n", __func__, cm->desc->fast_charger_type);
...
static int try_charger_enable_by_psy(struct charger_manager *cm, bool enable)
{
	...
	for (i = 0; desc->psy_charger_stat[i]; i++) {
		...
		if (!psy) {
			...
			dev_info(cm->dev, "try_charger_enable_by_psy : find power supply \"%s\"  enable=%d\n",
				desc->psy_charger_stat[i], enable);
		}
...
/*		val.intval = 4600;
	...
	if(prj_name == 2 || prj_name ==3|| prj_name ==20){
		sgm4154x_set_prechrg_curr(pinfo,300);
	}
	sgm41512_set_prechrg_curr(sgm41512_info,300);
...
/*odm.hq.bsp.luowenjiang@huaqin.com 2021.11.26 modify for recharge start*/
#define TEMPERATURE_LEVE_N20	-200
#define TEMPERATURE_LEVE_0		0
#define TEMPERATURE_LEVE_P5		50
#define TEMPERATURE_LEVE_P53	530
#define VRES_10MV	100000
#define VRES_20MV	200000
#define VRES_30MV	300000

static void cm_vbatt_full_recharge(struct charger_manager *cm)
{
	struct charger_desc *desc = cm->desc;
	int tbat;
	int ret = 0;
	...
	if(tbat >= TEMPERATURE_LEVE_N20 && tbat < TEMPERATURE_LEVE_0) {
		...
		if(tbat >= 450){
			diff = CM_HIGH_TEMP_STATUS_CV - batt_ocv;
			if(diff >= VRES_10MV){
				is_full_warm = false;
				try_charger_restart(cm);
			}
		}else if( diff >= VRES_10MV )
			try_charger_restart(cm);
	}
}
/*odm.hq.bsp.luowenjiang@huaqin.com 2021.11.26 modify for recharge start*/
...
static void cm_check_charge_voltage(struct charger_manager *cm)
{
	...
	} else if (!cm->charger_enabled && charge_vol <= (desc->charge_voltage_max - desc->charge_voltage_drop) &&
		   (cm->charging_status & CM_CHARGE_VOLTAGE_ABNORMAL)) {
...
out:
	last_temp = cur_temp;
	dev_dbg(cm->dev, "%s: jeita status:(%d) %d %d, temperature:%d, jeita_size:%d\n",
...
void cm_set_cc_open(void)
{
	printk(KERN_ERR "[%s]: cm_set_cc_open\n", __func__);
	if(psc != NULL)
		sc27xx_typec_set_cc_open(psc);
}

void cm_set_cc_default(void)
{
	printk(KERN_ERR "[%s]: cm_set_cc_default\n", __func__);
	if(psc != NULL)
		sc27xx_typec_set_cc_default(psc);
}
...
void cm_set_ship_mode(bool value)
{
	...
	if(pinfo != NULL){
		...
		if(value){
			...
			ret = sgm4154x_enable_shipmode(pinfo,false);
			if(ret < 0)
			printk("[%s]: sgm4154x disable shipmode fail[%d]\n", __func__, value);
		}
	}else
		return;
}
...
static bool cm_charger_is_support_fchg(struct charger_manager *cm)
{
	...
	for (i = 0; desc->psy_fast_charger_stat[i]; i++) {
		...
		dev_err(cm->dev, "%s eddyhua val.intval = %d\n", __func__, val.intval);
		if (!ret) {
...
static int cm_get_capacity_level(struct charger_manager *cm)
{
	...
	if (ocv_uv <= CM_CAPACITY_LEVEL_CRITICAL_VOLTAGE)
	{
		if (get_ibat_now_uA(cm, &batt_uA)) {
			dev_err(cm->dev, "%s, batt_uA error.\n", __func__);
		    return level;
	    }
		dev_info(cm->dev, "%s, batt_uA [%d]\n", __func__, batt_uA);
	}

	

	if (level == POWER_SUPPLY_CAPACITY_LEVEL_CRITICAL && is_charging(cm) &&
	    ((ocv_uv > CM_CAPACITY_LEVEL_CRITICAL_VOLTAGE) || (batt_uA > 300000)) )
		level = POWER_SUPPLY_CAPACITY_LEVEL_LOW;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (is_charging(cm)) {
			...
			else if(notify_code & (CM_CHARGE_TEMP_OVERHEAT | CM_CHARGE_TEMP_COLD) || is_full_warm)
				cm->battery_status = POWER_SUPPLY_STATUS_CHARGING;
			else if (g_batt_jeita_temp & CHG_BAT_HIG_TEMP_STATUS) {
				cm->battery_status = POWER_SUPPLY_STATUS_CHARGING;
			}
			else if (cm->charging_status & CM_CHARGE_DURATION_ABNORMAL) {
				cm->battery_status = POWER_SUPPLY_STATUS_CHARGING;
				dev_warn(cm->dev, "Charging duration abnormal: battery_status CHARGING\n");
	        }
			else
		...
		dev_info(cm->dev, "%s  cm->battery_status=%d\n", __func__, cm->battery_status);
		break;
		     ...
		     const union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
	dev_info(cm->dev, "thermal set charge ibat limit:   %d\n", val->intval);
		cm->desc->thm_info.thm_pwr = 0;
	...
	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		/* The ChargerIC with linear charging cannot set Ibus, only Ibat. */
		dev_info(cm->dev, "thermal set charge ibat limit2:   %d\n", val->intval);
		if (cm->desc->thm_info.need_calib_charge_lmt) {
			cm->cm_charge_vote->vote(cm->cm_charge_vote, true,
					 SPRD_VOTE_TYPE_IBAT,
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT:
		...
		cm->desc->thm_info.thm_pwr = val->intval;
		 ...
		 //  !!!!!!!just for test  must be remove  !!!!!!!!!!!!!!!!!!!!!!!
		// cm->desc->thm_info.thm_pwr = 20000;  //  just for test
		//  !!!!!!!just for test  must be remove  !!!!!!!!!!!!!!!!!!!!!!!
                     ...
                     const union power_supply_propval *val)
{
        ...
        case POWER_SUPPLY_PROP_OTG_SWITCHER:
			...
			if(prj_name == 2 || prj_name == 3|| prj_name == 20){ //nicky typec
				if(val->intval){
					if (otg_switch_state != val->intval)cm_set_otg_switch_status(true);
				}else{
					if (otg_switch_state != val->intval)cm_set_otg_switch_status(false);
				}				
			}else{
		...
		case POWER_SUPPLY_PROP_COOL_DOWN:
			...
			#else
			dev_info(&psy->dev, "[%s] POWER_SUPPLY_PROP_COOL_DOWN not implement !!! \n",__func__);
			#endif
			break;
    	case POWER_SUPPLY_PROP_SHIP_MODE:
		dev_info(&psy->dev, "set ship mode, ship_mode = %d\n", val->intval);
		if(prj_name == 2 || prj_name == 3|| prj_name == 20){
			ship_mode = val->intval;
		}
		break;			
...
static void cm_update_charger_type_status(struct charger_manager *cm)
{
	dev_err(cm->dev, "%s:  charger_type=%d\n", __func__, cm->desc->charger_type);
...
static int cm_init_thermal_data(struct charger_manager *cm, struct power_supply *fuel_gauge)
{
	...
	if (desc->usb1_zone) {
		cm->tzd_usb1 =
			thermal_zone_get_zone_by_name(desc->usb1_zone);
	}
	if (desc->usb2_zone) {
		cm->tzd_usb2 =
			thermal_zone_get_zone_by_name(desc->usb2_zone);
	}
...
static struct charger_desc *of_cm_parse_desc(struct device *dev)
{
	...
	of_property_read_string(np, "cm-usb1-zone", &desc->usb1_zone);
	of_property_read_string(np, "cm-usb2-zone", &desc->usb2_zone);
...
static void cm_jeita_temp_check(struct charger_manager *cm)
{
	...
	//	notify_code |= CHG_BAT_HIG_TEMP_STATUS;
		g_batt_jeita_temp |= CHG_BAT_HIG_TEMP_STATUS;
		...
		g_batt_jeita_temp &= ~CHG_BAT_HIG_TEMP_STATUS;
		dev_info(cm->dev, "%s: batt_temp:%d form hot temp restore normal temp 222\n", __func__, cur_temp);
...
#endif
	int t1,t2;
	int vbus,chg_cur,batt_uV;
	int temp_add_for_charge;
#ifdef CONFIG_THERMAL
	temp_add_for_charge=0;
	if (cm!=NULL&&!get_charger_voltage(cm, &vbus)){
		//pr_err("vbus = %d\n",vbus);
		if(vbus >= 3000000) {
			if(!get_charger_current(cm, &chg_cur)){
			//	pr_err("ibatter = %d\n",chg_cur);
				if(!get_vbat_now_uV(cm, &batt_uV)){
			//		pr_err("batt_uV = %d\n",batt_uV);
					vbus/=1000;
				    chg_cur/=1000;
					batt_uV/=1000;
					chg_cur=chg_cur*batt_uV*10/(9*vbus);
			//		pr_err("ibus = %d\n",chg_cur);
					temp_add_for_charge=chg_cur*75/100;//sub for charge
					}
				}
			
		}
	}
//	pr_err("temp_add_for_charge = %d\n",temp_add_for_charge);

	if (cm->tzd_usb1) {
		rc = thermal_zone_get_temp(cm->tzd_usb1, &t1);
		if (rc){
			pr_err("ERROR:%s:%u Failed to get usb ADC temp,rc = %d\n",__func__,__LINE__,rc);
			return rc;
		}
		if(t1>100*1000)
			temp_add_for_charge*=11;
//		pr_err("temp_add_for_charge2 = %d\n",temp_add_for_charge);

		t1-=temp_add_for_charge;
		t1/=1000;
	}
	if (cm->tzd_usb2) {
		rc = thermal_zone_get_temp(cm->tzd_usb2, &t2);
		if (rc){
			pr_err("ERROR:%s:%u Failed to get usb ADC temp,rc = %d\n",__func__,__LINE__,rc);
			return rc;
		}
		...
		t2-=temp_add_for_charge;
		t2/=1000;
	}
	//pr_err("cm_get_usbs_temp t1 = %d\n",t1);
...
#define CONTROL_CHARGE_BY_GPIO

#ifdef CONTROL_CHARGE_BY_GPIO
...
int cm_usb_temp_check_thread(void *data)
{
    ...
    if (g_bat_fix_temp) {
		dev_info(cm->dev,"ERROR:%s:%u g_bat_fix_temp return!! \n",__func__,__LINE__);
		return 0;	
	}
	...
	while (!kthread_should_stop()) {
		...
		if (vbus == 0 ) {
			...
			pr_info("[ADC] vbus is zreo check again\n");
			goto recheck;
		}
		...
		if ((batt_temp < TEMP_50C && usb_temp_1 >= TEMP_57C && usb_temp_1 <= TEMP_100C ) ||
			...
			schedule_delayed_work(&cm->usb_temp_cc_work, msecs_to_jiffies(USBTEMP_CC_OPEN_INTERVAL));
...
static void cm_usb_temp_cc_work(struct work_struct *work) 
{
	pr_info("[%s:%u zq cm_usb_temp_cc_work in\n",__func__,__LINE__);
	cm_set_cc_default();
	//set to sink
	cm_set_otg_switch_status(false);
}
...
#define SET_FAST_CHECK_HI_BATTERY_TEMPERATURE         700 // 400// 700
#define SET_POWEROFF_90S_HI_BATTERY_TEMPERATURE       770 // 470// 770
#define SET_POWEROFF_30S_HI_BATTERY_TEMPERATURE       850 // 500// 850

//static struct delayed_work hi_temp_bat_fastcheck_work;
static struct {
	bool set_poweroff_flag;
	bool fast_check_flag;

	bool step90S_record_flag;
	u32 step90S_continued_time;
	struct timespec64 step90S_start_time;

	bool step30S_record_flag;
	u32 step30S_continued_time;
	struct timespec64 step30S_start_time;
}batt_hi_temp;

static void battery_high_temperature_toset_poweroff_check(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct charger_manager *cm = container_of(dwork,
						  struct charger_manager,
						  hi_temp_bat_fastcheck_work);
	int ret, cur_temp;
	...
	ret = cm_get_battery_temperature_by_psy(cm, &cur_temp);
	if (ret) {
		dev_err(cm->dev, "%sfailed to get battery temperature\n", __func__);
		return;
	}
	cur_time = ktime_to_timespec64(ktime_get_boottime());
	dev_info(cm->dev, "%s  bat_temp[%d] cur_time[%d]\n", __func__, cur_temp, cur_time.tv_sec);

	if (cur_temp > SET_POWEROFF_30S_HI_BATTERY_TEMPERATURE) {
		if (0 == batt_hi_temp.step30S_record_flag) {
			batt_hi_temp.step30S_start_time.tv_sec = cur_time.tv_sec;
		}
		batt_hi_temp.step30S_record_flag = 1;
		batt_hi_temp.step30S_continued_time = cur_time.tv_sec - batt_hi_temp.step30S_start_time.tv_sec;
		if (batt_hi_temp.step30S_continued_time >= 30)batt_hi_temp.set_poweroff_flag = 1;
		dev_info(cm->dev, "%s  step30S_continued_time[%d]\n", __func__, batt_hi_temp.step30S_continued_time);

		if (0 == batt_hi_temp.step90S_record_flag) {
			batt_hi_temp.step90S_start_time.tv_sec = cur_time.tv_sec;
		}
		batt_hi_temp.step90S_record_flag = 1;
		batt_hi_temp.step90S_continued_time = cur_time.tv_sec - batt_hi_temp.step90S_start_time.tv_sec;
		if (batt_hi_temp.step90S_continued_time >= 90)batt_hi_temp.set_poweroff_flag = 1;
		dev_info(cm->dev, "%s  step90S_continued_time[%d]\n", __func__, batt_hi_temp.step90S_continued_time);
	}
	else if (cur_temp > SET_POWEROFF_90S_HI_BATTERY_TEMPERATURE) {
		if (0 == batt_hi_temp.step90S_record_flag) {
			batt_hi_temp.step90S_start_time.tv_sec = cur_time.tv_sec;
		}
		batt_hi_temp.step90S_record_flag = 1;
		batt_hi_temp.step30S_record_flag = 0;
		batt_hi_temp.step90S_continued_time = cur_time.tv_sec - batt_hi_temp.step90S_start_time.tv_sec;
		if (batt_hi_temp.step90S_continued_time >= 90)batt_hi_temp.set_poweroff_flag = 1;
		dev_info(cm->dev, "%s  step90S_continued_time[%d]\n", __func__, batt_hi_temp.step90S_continued_time);
	}
	else {
		batt_hi_temp.step90S_record_flag = 0;
		batt_hi_temp.step30S_record_flag = 0;
		batt_hi_temp.set_poweroff_flag = 0;
	}

	if (batt_hi_temp.set_poweroff_flag == 1) {  // set poweroff
	    dev_err(cm->dev, "%s set power off !!! .\n", __func__);
		batt_hi_temp.step90S_record_flag = 0;
		batt_hi_temp.step30S_record_flag = 0;
		batt_hi_temp.set_poweroff_flag = 0;
		//orderly_poweroff(true);
	...
	if (cur_temp > SET_FAST_CHECK_HI_BATTERY_TEMPERATURE)schedule_delayed_work(&cm->hi_temp_bat_fastcheck_work, msecs_to_jiffies(1000));
...
static void cm_batt_works(struct work_struct *work)
{
...
#ifdef USE_SW_CONTROL_HITEMP_BAT_SETPOWEROFF
    //if (is_charger_mode)dev_info(cm->dev, "is_charger_mode skip FAST_CHECK_HI_BATTERY_TEMPERATURE.\n");
	
    //if (cur_temp > SET_FAST_CHECK_HI_BATTERY_TEMPERATURE) {
	if ( (cur_temp > SET_FAST_CHECK_HI_BATTERY_TEMPERATURE) && (false == is_charger_mode) ){
		if (batt_hi_temp.fast_check_flag == 0) {
			batt_hi_temp.fast_check_flag = 1;
			batt_hi_temp.step90S_record_flag = 0;
			batt_hi_temp.step30S_record_flag = 0;
			//dev_err(cm->dev, "hi_temp_bat_fastcheck_wake_lock --> set\n");
			//__pm_stay_awake(&cm->hi_temp_bat_fastcheck_wake_lock);
			schedule_delayed_work(&cm->hi_temp_bat_fastcheck_work, msecs_to_jiffies(200));
		}
	}
	else {
		if (batt_hi_temp.fast_check_flag == 1){
			//dev_err(cm->dev, "hi_temp_bat_fastcheck_wake_lock --> relax\n");
			//__pm_relax(&cm->hi_temp_bat_fastcheck_wake_lock);			
		}
		batt_hi_temp.fast_check_flag = 0;
	}
#endif
	...
	#ifdef CONFIG_HQ_USB_TEMP_CHECK
	if (false == ccali_mode && (prj_name == 2 || prj_name == 3 || prj_name == 20)) {
	...
	dev_info(cm->dev, "vbat: %d, vbat_avg: %d, OCV: %d, ibat: %d, ibat_avg: %d, ibus: %d,"
		 ...
		 " thm_pwr: %d, is_fchg: %d, fchg_en: %dforce_set_full = , tflush: %d, tperiod: %d, prj_name: %d usb_temp_1: %d usb_temp_2: %d\n",
		 ...
		 cm->desc->is_fast_charge, cm->desc->enable_fast_charge, flush_time, period_time, prj_name,usb_temp_1,usb_temp_2);
	...
	/*odm.hq.bsp.luowenjiang@huaqin.com 2021.11.26 modify for recharge start*/
       if (is_full_charged(cm))
               cm_vbatt_full_recharge(cm);
    /*odm.hq.bsp.luowenjiang@huaqin.com 2021.11.26 modify for recharge end*/
...
#ifndef USE_SW_CONTROL_HITEMP_BAT_SETPOWEROFF
...
static ssize_t show_tn_chgtype(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct charger_manager *cm = dev_get_drvdata(dev);

	dev_err(cm->dev,"[show_tn_chgtype]  cm->desc->charger_type=%d\n", cm->desc->charger_type);

	return sprintf(buf, "%d\n", cm->desc->charger_type);
}

static DEVICE_ATTR(runin_stop, 0644, show_runin_stop, store_runin_stop);
static DEVICE_ATTR(tn_chgtype, 0644, show_tn_chgtype, NULL);

static struct attribute *bbat_attributes[] = {
    &dev_attr_runin_stop.attr,
	&dev_attr_tn_chgtype.attr,
    NULL
};
...
static int charger_manager_probe(struct platform_device *pdev)
{
	...
	#ifdef USE_SW_CONTROL_HITEMP_BAT_SETPOWEROFF
	INIT_DELAYED_WORK(&cm->hi_temp_bat_fastcheck_work, battery_high_temperature_toset_poweroff_check);
	//wakeup_source_init(&cm->hi_temp_bat_fastcheck_wake_lock, "hi-temp-bat-fastcheck");	
	batt_hi_temp.fast_check_flag = 0;
	batt_hi_temp.set_poweroff_flag = 0;
	#endif
	
	dev_err(&pdev->dev, "[charger_manager_probe]  bbat_mode[%d] ccali_mode[%d]\n", bbat_mode, ccali_mode);
	/*odm.hq.bsp.luowenjiang@huaqin.com 2021.11.04 modify for usb temp protection start*/
	#ifdef CONFIG_HQ_USB_TEMP_CHECK
	if ( (ccali_mode == false) && (prj_name == 2 || prj_name == 3|| prj_name == 20)) {
		my_cm = cm;
		#ifndef CONFIG_THERMAL
		...
		INIT_DELAYED_WORK(&cm->usb_temp_cc_work, cm_usb_temp_cc_work);
	}
	#endif
	/*odm.hq.bsp.luowenjiang@huaqin.com 2021.11.04 modify for usb temp protection end*/
	psy_cfg.of_node = np;
...
static int charger_manager_remove(struct platform_device *pdev)
{
	...
	#ifdef USE_SW_CONTROL_HITEMP_BAT_SETPOWEROFF
	cancel_delayed_work_sync(&cm->hi_temp_bat_fastcheck_work);
	#endif
...
{
	struct charger_manager *cm = platform_get_drvdata(pdev);
	printk("%s   ship_mode:%d", __func__, ship_mode);

	if (cm->desc->uvlo_trigger_cnt < CM_UVLO_CALIBRATION_CNT_THRESHOLD)
		set_batt_cap(cm, cm->desc->cap);
#if 1
		if(ship_mode == 1)cm_set_ship_mode(true);
#endif
}
...
static int cm_suspend_prepare(struct device *dev)
{
	...
	if (cm_timer_set) {
		...
		cancel_delayed_work_sync(&cm->uvlo_work);
		#ifdef USE_SW_CONTROL_HITEMP_BAT_SETPOWEROFF
		cancel_delayed_work(&cm->hi_temp_bat_fastcheck_work);
		#endif
	}
