...
//add for GPIO82 ---> GPIO170 yindongsheng begin
extern char *saved_command_line;
static int pcba_version = 6;
static char *gpio82_pctl_name = "gpio82_active";
struct pinctrl *dvdd_pinctrl;
...
static int get_pcb_version(void)
{
	...
	if(ptr != 0){
		...
		pcba_version = simple_strtol(ptr, NULL, 13);
	}else{
		pcba_version = 0;
	}

	pr_err("pcba_version:%d\n", pcba_version);
	return 0;
}
//add for GPIO82 ---> GPIO170 yindongsheng end
				...
				*sensor_info)
{
	...
	for (i = 0; i < SPRD_SENSOR_GPIO_TAG_MAX; i++) {
		...
		if (gpio_is_valid(sensor_info->gpio_tab[i])) {
			...
			//add for GPIO82 ---> GPIO170 yindongsheng begin
			if ((pcba_version < 6) && (i == 9)) {//dvdd-gpios
				dvdd_pinctrl = devm_pinctrl_get(dev);

				if(!IS_ERR(dvdd_pinctrl)) {
					pinctrl_state_active = pinctrl_lookup_state(dvdd_pinctrl, gpio82_pctl_name);
					pr_info("pinctrl_state %s\n", gpio82_pctl_name);

					if (IS_ERR(pinctrl_state_active)) {
						pr_err("Cannot find pinctrl %s!\n", gpio82_pctl_name);
					}
				} else {
			...
			const struct i2c_device_id *id)
{
	...
	if (of_device_is_compatible(dev->of_node, "sprd,sensor-main")) {
		get_pcb_version();//add for GPIO82 ---> GPIO170 yindongsheng
...
int sprd_sensor_set_voltage_by_gpio(int sensor_id, unsigned int val, int type)
{
	...
	if (gpio_is_valid(gpio_id)) {
		//add for GPIO82 ---> GPIO170 yindongsheng being
		if ((SPRD_SENSOR_DVDD_GPIO_TAG_E == type) && (pcba_version < 6)) {
			if(!(IS_ERR(dvdd_pinctrl) || IS_ERR(pinctrl_state_active))) {
				pinctrl_select_state(dvdd_pinctrl, pinctrl_state_active);
			} else {
				pr_err("%s, dvdd_pinctrl or pinctrl_state_active error!!!\n", __func__);
			}
		}
		//add for GPIO82 ---> GPIO170 yindongsheng end
	...
	pr_err("sensor %d gpio_id:%d vdd val %d\n", sensor_id, gpio_id, val);
