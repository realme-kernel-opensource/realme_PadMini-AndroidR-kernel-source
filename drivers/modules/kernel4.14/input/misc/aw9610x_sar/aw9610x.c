...
static int32_t aw9610x_cfg_update(struct aw9610x *aw9610x)
{
	...
	if (aw9610x->firmware_flag == true) {
		if(strstr(saved_command_line, "wifionly=1"))
			snprintf(aw9610x->cfg_name, sizeof(aw9610x->cfg_name),
					"aw9610x_%d_wifi.bin", aw9610x->sar_num);
		else
			snprintf(aw9610x->cfg_name, sizeof(aw9610x->cfg_name),
					"aw9610x_%d_cell.bin", aw9610x->sar_num);
...
static ssize_t aw9610x_batch_set(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	return count;
...
static ssize_t aw9610x_batch_get(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	return len;
...
static ssize_t aw9610x_flush_set(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	return count;
...
static ssize_t aw9610x_flush_get(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	return len;
...
static ssize_t aw9610x_sensorhub_get(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	return len;
}
static ssize_t aw9610x_sensorhub_set(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	return count;
...
static ssize_t aw9610x_delay_get(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	return len;
}
static ssize_t aw9610x_delay_set(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
	return count;
...
static DEVICE_ATTR(batch, 0644, aw9610x_batch_get,
						aw9610x_batch_set);
static DEVICE_ATTR(flush, 0644, aw9610x_flush_get,
						aw9610x_flush_set);
static DEVICE_ATTR(delay, 0644, aw9610x_delay_get,
						aw9610x_delay_set);
static DEVICE_ATTR(enable, 0644, aw9610x_enable_get,
						aw9610x_enable_set);
static DEVICE_ATTR(sensorhub, 0644, aw9610x_sensorhub_get,
						aw9610x_sensorhub_set);
...
static struct class sarsense_class = {
	.name			= "sarsense",
	.owner			= THIS_MODULE,
};
...
aw9610x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	ret = class_register(&sarsense_class);
	if (ret < 0) {
		pr_err("Create fsys class failed (%d)\n", ret);
	}
	ret = class_create_file(&sarsense_class, &class_attr_enable);
	if (ret < 0) {
		pr_err("Create enable file failed (%d)\n", ret);
	}
	ret = class_create_file(&sarsense_class, &class_attr_batch);
	if (ret < 0) {
		pr_err("Create batch file failed (%d)\n", ret);
	}
