...
{
	char *ptr = NULL;
	ptr = strstr(saved_command_line,"androidboot.product.prjversion");
	if(ptr != NULL) {
		ptr +=strlen("androidboot.product.prjversion=");
		printk("[kernel] androidboot.product.prjversion5G: &s",ptr);
	}
	return ptr;
}
...
int sprdwl_get_fw_info(struct sprdwl_priv *priv)
{
	...
	if (!ret && r_len) {
		...
		if (prj_name != NULL && (strncmp(prj_name,"216",3) == 0)) {
			priv->fw_capa &= ~(1 << 0);
