...
static int gadc_thermal_get_temp(void *data, int *temp)
{
	...
	int val;
	int ret;
	if (gpio_is_valid(gti->hwen_gpio)) {
		if (gti->hwen_channel) {
			//pr_info("hwen pin is going to be high!---<%d>\n", gti->hwen_channel);
			gpio_set_value(gti->hwen_gpio, true);
		} else {
			//pr_info("hwen pin is going to be low!---<%d>\n", gti->hwen_channel);
			gpio_set_value(gti->hwen_gpio, false);
		}
	}//tn add 
						 ...
						 struct gadc_thermal_info *gti)
{
 ...
 static int gadc_thermal_read_hwen_gpio(struct device *dev,
						  struct gadc_thermal_info *gti)
 {
	struct device_node *np = dev->of_node;
 	gti->hwen_gpio = of_get_named_gpio(np, "hwen-gpio", 0);

	if (gpio_is_valid(gti->hwen_gpio)) {
			dev_err(dev, "hwen_gpio is: %d\n", gti->hwen_gpio);
		of_property_read_u32(np, "hwen-channel", &gti->hwen_channel);
	return 0;
	}
	return -1;
}
...
static int gadc_thermal_probe(struct platform_device *pdev)
{
	...
	gadc_thermal_read_hwen_gpio(&pdev->dev, gti);
