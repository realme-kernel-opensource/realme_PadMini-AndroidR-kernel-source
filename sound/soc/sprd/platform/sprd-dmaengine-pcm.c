...
#define sp_asoc_pr_info pr_debug
...
static int sprd_pcm_open(struct snd_pcm_substream *substream)
{
	...
	memset_io(rtd->dma_cfg_virt[0], 0, size_inout);
	sp_asoc_pr_info("rtd->dma_cfg_virt[0] =%#lx, rtd->dma_cfg_phy[0] =%#lx,",
		(unsigned long)rtd->dma_cfg_virt[0],
		(unsigned long)rtd->dma_cfg_phy[0]);
	sp_asoc_pr_info("rtd->dma_cfg_virt[1]=%#lx,rtd->dma_cfg_phy[1]=%#lx,",
		(unsigned long)rtd->dma_cfg_virt[1],
		(unsigned long)rtd->dma_cfg_phy[1]);

	sp_asoc_pr_info("runtime->hw.periods_max*(sizeof(struct sprd_dma_cfg)=%zd,",
		runtime->hw.periods_max*(sizeof(struct sprd_dma_cfg)));
	sp_asoc_pr_info("sizeof(struct sprd_dma_cfg) = %zd, size_inout=%u\n",
				    ...
				    int playback, int interleaved)
{
	...
	if (interleaved) {
		...
		sp_asoc_pr_info("desc->src_step  =%u, des_step=%u, datawidth =%u\n",
		       desc->src_step,
		       desc->des_step,
		       desc->datawidth);
	} else {
	...
	struct snd_pcm_substream *substream, int ch_cnt)
{
	...
	for (i = 0; i < ch_cnt; i++) {
		...
		rtd->transform_type = sprd_pcm_get_transf_type(substream,
			dma_chn_request);
		sp_asoc_pr_info("Chan%d DMA ID=%d\n",
			       ...
			       dma_addr_t *dma_buff_phys)
{
	...
	for (i = 0; i < ch_cnt; i++) {
		...
		sp_asoc_pr_info("src_dw=%d, dst_dw=%d frag=%lx, burst:%d, slave_id=%d, step=%d, ll_cfg_virt_addr=%p, ll_cfg_phy_addr=%ld ,dst_addr:%ld, src_addr:%ld,chan=%d\n",
	...
	do {
		for (i = 0; i < ch_cnt; i++) {
			...
			sp_asoc_pr_info("sg_dma_len=%zu, periods=%zu, sg_dma_address=%p, flag:%lx, chan=%d node=%d\n",
