...
extern int aw87xxx_audio_mode_set( int id, int mode);
extern int aw87xxx_audio_mode_get( int id);
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
    ...
    if((BOARD_FUNC_SPK == id) || (BOARD_FUNC_SPK1 == id)) {
        //if mode not changed, do nothing
    	if (aw87xxx_audio_mode_get(id) == ucontrol->value.integer.value[0])
    		return 0;
    } else {
    ...
    if((BOARD_FUNC_SPK == id) || (BOARD_FUNC_SPK1 == id)) {
        //if mode is changed, reopen pa
        if(aw87xxx_audio_mode_get(id) && board.func_switch[id]) {
            board.func_switch[id] = SWITCH_FUN_OFF;
            board_ext_pin_control(&card->dapm, id, id + 1);
        }
        aw87xxx_audio_mode_set(id, ucontrol->value.integer.value[0]);
        if(ucontrol->value.integer.value[0] > 0) {
            board.func_switch[id] = SWITCH_FUN_ON;
        }
    }
...
#define BOARD_CODEC_FUNC_FOR_AW87XXX(xname, xreg) \
	SOC_SINGLE_EXT(xname, FUN_REG(xreg), \
		0, 5, 0, board_func_get, board_func_set)
...
static const struct snd_kcontrol_new _sprd_asoc_card_controls[] = {
	BOARD_CODEC_FUNC_FOR_AW87XXX("Speaker Function", BOARD_FUNC_SPK),
	BOARD_CODEC_FUNC_FOR_AW87XXX("Speaker1 Function", BOARD_FUNC_SPK1),
